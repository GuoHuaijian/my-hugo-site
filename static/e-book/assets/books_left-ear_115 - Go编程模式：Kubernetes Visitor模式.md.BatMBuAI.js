import{_ as a,o as e,c as p,ae as i,j as n}from"./chunks/framework.Iv6F95cJ.js";const f=JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"books/left-ear/115 - Go编程模式：Kubernetes Visitor模式.md","filePath":"books/left-ear/115 - Go编程模式：Kubernetes Visitor模式.md"}'),t={name:"books/left-ear/115 - Go编程模式：Kubernetes Visitor模式.md"};function l(r,s,o,c,d,u){return e(),p("div",null,[...s[0]||(s[0]=[i(`<p>你好，我是陈皓，网名左耳朵耗子。</p><p>这节课，我们来重点讨论一下，Kubernetes 的 <code>kubectl</code> 命令中的使用到的一个编程模式：Visitor（其实，<code>kubectl</code> 主要使用到了两个，一个是Builder，另一个是Visitor）。</p><p>本来，Visitor 是面向对象设计模式中一个很重要的设计模式（可以看下Wikipedia <a href="https://en.wikipedia.org/wiki/Visitor_pattern" target="_blank" rel="noreferrer">Visitor Pattern词条</a>），这个模式是将算法与操作对象的结构分离的一种方法。这种分离的实际结果是能够在不修改结构的情况下向现有对象结构添加新操作，是遵循开放/封闭原则的一种方法。这节课，我们重点学习一下 <code>kubelet</code> 中是怎么使用函数式的方法来实现这个模式的。</p><h2 id="一个简单示例" tabindex="-1">一个简单示例 <a class="header-anchor" href="#一个简单示例" aria-label="Permalink to &quot;一个简单示例&quot;">&amp;ZeroWidthSpace;</a></h2><p>首先，我们来看一个简单设计模式的Visitor的示例。</p><ul><li>我们的代码中有一个<code>Visitor</code>的函数定义，还有一个<code>Shape</code>接口，这需要使用 <code>Visitor</code>函数作为参数。</li><li>我们的实例的对象 <code>Circle</code>和 <code>Rectangle</code>实现了 <code>Shape</code> 接口的 <code>accept()</code> 方法，这个方法就是等外面给我们传递一个Visitor。</li></ul><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>package main</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p>可以看到，</p><ul><li>有一个 <code>VisitorFunc</code> 的函数类型的定义；</li><li>一个 <code>Visitor</code> 的接口，其中需要 <code>Visit(VisitorFunc) error</code> 的方法（这就像是我们上面那个例子的 <code>Shape</code> ）；</li><li>最后，为<code>Info</code> 实现 <code>Visitor</code> 接口中的 <code>Visit()</code> 方法，实现就是直接调用传进来的方法（与前面的例子相仿）。</li></ul><p>我们再来定义几种不同类型的 Visitor。</p><h3 id="name-visitor" tabindex="-1">Name Visitor <a class="header-anchor" href="#name-visitor" aria-label="Permalink to &quot;Name Visitor&quot;">&amp;ZeroWidthSpace;</a></h3><p>这个Visitor 主要是用来访问 <code>Info</code> 结构中的 <code>Name</code> 和 <code>NameSpace</code> 成员：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>type NameVisitor struct {</span></span>
...
<span class="line"><span>  })</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到，在这段代码中：</p><ul><li>声明了一个 <code>NameVisitor</code> 的结构体，这个结构体里有一个 <code>Visitor</code> 接口成员，这里意味着多态；</li><li>在实现 <code>Visit()</code> 方法时，调用了自己结构体内的那个 <code>Visitor</code>的 <code>Visitor()</code> 方法，这其实是一种修饰器的模式，用另一个Visitor修饰了自己（关于修饰器模式，可以复习下<a href="https://time.geekbang.org/column/article/332608" target="_blank" rel="noreferrer">第113讲</a>）。</li></ul><h3 id="other-visitor" tabindex="-1">Other Visitor <a class="header-anchor" href="#other-visitor" aria-label="Permalink to &quot;Other Visitor&quot;">&amp;ZeroWidthSpace;</a></h3><p>这个Visitor主要用来访问 <code>Info</code> 结构中的 <code>OtherThings</code> 成员：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>type OtherThingsVisitor struct {</span></span>
<span class="line"><span>  visitor Visitor</span></span>
<span class="line"><span>}</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>    return err</span></span>
<span class="line"><span>  })</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>实现逻辑同上，我就不再重新讲了。</p><h3 id="log-visitor" tabindex="-1">Log Visitor <a class="header-anchor" href="#log-visitor" aria-label="Permalink to &quot;Log Visitor&quot;">&amp;ZeroWidthSpace;</a></h3><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>type LogVisitor struct {</span></span>
<span class="line"><span>  visitor Visitor</span></span>
<span class="line"><span>}</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>    return err</span></span>
<span class="line"><span>  })</span></span>
<span class="line"><span>}</span></span></code></pre></div><h3 id="使用方代码" tabindex="-1">使用方代码 <a class="header-anchor" href="#使用方代码" aria-label="Permalink to &quot;使用方代码&quot;">&amp;ZeroWidthSpace;</a></h3><p>现在，我们看看使用上面的代码：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>func main() {</span></span>
...
<span class="line"><span></span></span>
<span class="line"><span>v.Visit(LoadFile)</span></span></code></pre></div><p>是不是比之前的那个简单？需要注意的是，这个<code>DecoratedVisitor</code> 同样可以成为一个Visitor来使用。上面的这些代码全部存在于 <code>kubectl</code> 的代码中，只要你看懂了这里面的代码逻辑，就一定能看懂 <code>kubectl</code> 的代码。</p><p>好了，这节课就到这里。如果你觉得今天的内容对你有所帮助，欢迎你帮我分享给更多人。 精选留言（4） lumence 👍（4） 💬（1）使用装饰器，是不是该这样写哦 type VisitorDecorator func(VisitorFunc) VisitorFunc</p><p>type DecoratedVisitor struct { visitor Visitor decorators []VisitorDecorator }</p><p>func NewDecoratedVisitor(v Visitor, fn ...VisitorDecorator) Visitor { if len(fn) == 0 { return v } return DecoratedVisitor{v, fn} }</p><p>// Visit implements Visitor func (v DecoratedVisitor) Visit(fn VisitorFunc) error { decoratorLen := len(v.decorators) for i := range v.decorators { d := v.decorators[decoratorLen-i-1] fn = d(fn) } return fn(v.visitor.(*Info), nil) }2021-02-25hunknownz 👍（0） 💬（0）耗子哥，看最后「装饰器重构」这块，我的理解可能出了问题，你确认下。</p><p>k8s 中对于 Visitor 这块有两种结构：</p><p>一个是 VisiterList，它的 Visit 方法负责横向聚合各个 visitor 的 Visit 方法，把多个 visitor 合成一个 visitor，执行上存在父子 visitor 的关系，类似文章中把 LogVisitor，NameVisitor，OtherThingsVisitor 聚合成一个 visitor；</p><p>另一个是 DecoratedVisitor ，它的 Visit 方法负责纵向聚合多个 VisitorFunc 到一个 visitor 上，使 Visit 方法的逻辑更丰富，没有任何父子嵌套关系，它的 Visit 方法执行时先执行自己聚合的 decorators 方法们，然后再执行传入的 VisitorFunc。</p>`,60),n("p",{v:""},"我理解文章中「装饰器重构」是要完成 VisitorList 功能，但是使用了 DecoratedVisitor 这个结构，所以末尾的代码有一些执行不通。2022-05-10黑白灰 👍（0） 💬（4）var v Visitor = &info v = LogVisitor{v} v = NameVisitor{v} v = OtherThingsVisitor",-1),n("p",null,"请教一下go的，那个特性可以支持这种嵌套赋值2021-11-28shangyu 👍（0） 💬（0）耗子叔，最后那个NewDecoratedVisitor调用的传餐不对哦？2021-02-05",-1)])])}const V=a(t,[["render",l]]);export{f as __pageData,V as default};
