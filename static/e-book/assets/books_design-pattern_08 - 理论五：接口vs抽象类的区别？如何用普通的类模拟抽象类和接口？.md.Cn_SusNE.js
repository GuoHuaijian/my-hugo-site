import{_ as n,o as a,c as p,ae as e}from"./chunks/framework.Iv6F95cJ.js";const u=JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"books/design-pattern/08 - 理论五：接口vs抽象类的区别？如何用普通的类模拟抽象类和接口？.md","filePath":"books/design-pattern/08 - 理论五：接口vs抽象类的区别？如何用普通的类模拟抽象类和接口？.md"}'),l={name:"books/design-pattern/08 - 理论五：接口vs抽象类的区别？如何用普通的类模拟抽象类和接口？.md"};function i(t,s,c,o,r,g){return a(),p("div",null,[...s[0]||(s[0]=[e(`<p>在面向对象编程中，抽象类和接口是两个经常被用到的语法概念，是面向对象四大特性，以及很多设计模式、设计思想、设计原则编程实现的基础。比如，我们可以使用接口来实现面向对象的抽象特性、多态特性和基于接口而非实现的设计原则，使用抽象类来实现面向对象的继承特性和模板设计模式等等。</p><p>不过，并不是所有的面向对象编程语言都支持这两个语法概念，比如，C++这种编程语言只支持抽象类，不支持接口；而像Python这样的动态编程语言，既不支持抽象类，也不支持接口。尽管有些编程语言没有提供现成的语法来支持接口和抽象类，我们仍然可以通过一些手段来模拟实现这两个语法概念。</p><p>这两个语法概念不仅在工作中经常会被用到，在面试中也经常被提及。比如，“接口和抽象类的区别是什么？什么时候用接口？什么时候用抽象类？抽象类和接口存在的意义是什么？能解决哪些编程问题？”等等。</p><p>你可以先试着回答一下，刚刚我提出的几个问题。如果你对某些问题还有些模糊不清，那也没关系，今天，我会带你把这几个问题彻底搞清楚。下面我们就一起来看！</p><h2 id="什么是抽象类和接口-区别在哪里" tabindex="-1">什么是抽象类和接口？区别在哪里？ <a class="header-anchor" href="#什么是抽象类和接口-区别在哪里" aria-label="Permalink to &quot;什么是抽象类和接口？区别在哪里？&quot;">&amp;ZeroWidthSpace;</a></h2><p>不同的编程语言对接口和抽象类的定义方式可能有些差别，但差别并不会很大。Java这种编程语言，既支持抽象类，也支持接口，所以，为了让你对这两个语法概念有比较直观的认识，我们拿Java这种编程语言来举例讲解。</p><p><strong>首先，我们来看一下，在Java这种编程语言中，我们是如何定义抽象类的。</strong></p><p>下面这段代码是一个比较典型的抽象类的使用场景（模板设计模式）。Logger是一个记录日志的抽象类，FileLogger和MessageQueueLogger继承Logger，分别实现两种不同的日志记录方式：记录日志到文件中和记录日志到消息队列中。FileLogger和MessageQueueLogger两个子类复用了父类Logger中的name、enabled、minPermittedLevel属性和log()方法，但因为这两个子类写日志的方式不同，它们又各自重写了父类中的doLog()方法。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>// 抽象类</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>我们知道类中的方法必须包含实现，这个不符合接口的定义。但是，我们可以让类中的方法抛出MethodUnSupportedException异常，来模拟不包含实现的接口，并且能强迫子类在继承这个父类的时候，都去主动实现父类的方法，否则就会在运行时抛出异常。我们将构造函数设置成protected属性的，这样就能避免非同包下的类去实例化MockInterface。不过，这样还是无法避免同包中的类去实例化MockInterface。为了解决这个问题，我们可以学习Google Guava中@VisibleForTesting注解的做法，自定义一个注解，人为表明不可实例化。</p><p>刚刚我们讲了如何用抽象类来模拟接口，以及如何用普通类来模拟接口，那如何用普通类来模拟抽象类呢？这个问题留给你自己思考，你可以留言说说你的实现方法。</p><p>实际上，对于动态编程语言来说，还有一种对接口支持的策略，那就是duck-typing。我们在上一节课中讲到多态的时候也有讲过，你可以再回忆一下。</p><h2 id="如何决定该用抽象类还是接口" tabindex="-1">如何决定该用抽象类还是接口？ <a class="header-anchor" href="#如何决定该用抽象类还是接口" aria-label="Permalink to &quot;如何决定该用抽象类还是接口？&quot;">&amp;ZeroWidthSpace;</a></h2><p>刚刚的讲解可能有些偏理论，现在，我们就从真实项目开发的角度来看一下，在代码设计、编程开发的时候，什么时候该用抽象类？什么时候该用接口？</p><p>实际上，判断的标准很简单。如果我们要表示一种is-a的关系，并且是为了解决代码复用的问题，我们就用抽象类；如果我们要表示\b一种has-a关系，并且是为了解决抽象而非代码复用的问题，那我们就可以使用接口。</p><p>从类的继承层次上来看，抽象类是一种自下而上的设计思路，先有子类的代码重复，然后再抽象成上层的父类（也就是抽象类）。而接口正好相反，它是一种自上而下的设计思路。我们在编程的时候，一般都是先设计接口，再去考虑具体的实现。</p><h2 id="重点回顾" tabindex="-1">重点回顾 <a class="header-anchor" href="#重点回顾" aria-label="Permalink to &quot;重点回顾&quot;">&amp;ZeroWidthSpace;</a></h2><p>好了，今天内容就讲完了，我们一块来总结回顾一下，你需要掌握的重点内容。</p><p><strong>1.抽象类和接口的语法特性</strong></p><p>抽象类不允许被实例化，只能被继承。它可以包含属性和方法。方法既可以包含代码实现，也可以不包含代码实现。不包含代码实现的方法叫作抽象方法。子类继承抽象类，必须实现抽象类中的所有抽象方法。接口不能包含属性，只能声明方法，方法不能包含代码实现。类实现接口的时候，必须实现接口中声明的所有方法。</p><p><strong>2.抽象类和接口存在的意义</strong></p><p>抽象类是对成员变量和方法的抽象，是一种is-a关系，是为了解决代码复用问题。接口仅仅是对方法的抽象，是一种has-a关系，表示具有某一组行为特性，是为了解决解耦问题，隔离接口和具体的实现，提高代码的扩展性。</p><p><strong>3.抽象类和接口的应用场景区别</strong></p><p>什么时候该用抽象类？什么时候该用接口？实际上，判断的标准很简单。如果要表示一种is-a的关系，并且是为了解决代码复用问题，我们就用抽象类；如果要表示\b一种has-a关系，并且是为了解决抽象而非代码复用问题，那我们就用接口。</p><h2 id="课堂讨论" tabindex="-1">课堂讨论 <a class="header-anchor" href="#课堂讨论" aria-label="Permalink to &quot;课堂讨论&quot;">&amp;ZeroWidthSpace;</a></h2><ol><li>你熟悉的编程语言，是否有现成的语法支持接口和抽象类呢？具体是如何定义的呢？</li><li>前面我们提到，接口和抽象类是两个经常在面试中被问到的概念。学习完今天的内容之后，你是否对抽象类和接口有一个新的认识呢？如果面试官再让你聊聊接口和抽象类，你会如何回答呢？</li></ol><p>欢迎在留言区写下你的答案，和同学一起交流和分享。如果有收获，也欢迎你把这篇文章分享给你的朋友。 精选留言（15） helloworld 👍（41） 💬（17）『那又如何避免这个类被实例化呢？实际上很简单，我们只需要将这个类的构造函数声明为 protected 访问权限就可以了。』当把一个类的构造方法声明为protected后，在同一个包的其他类中还是可以new这个类的对象的。老师，是我想错了吗？请老师指正2019-11-21初学者 👍（9） 💬（1）抽象类做于代码复用，接口做于代码扩展2020-11-24ʸᵉ ᶠᵉᶮᵍ 👍（9） 💬（3）老师老师，我想补充一下，什么时候使用抽象类这个问题。文章中说如果要is-a关系就用抽象类，感觉这个答案有点不标准，is-a关系也可以用普通类来实现代码复用问题。更标准的答案是不是如果既有代码复用问题，子类又一定要实现多态关系的时候才用抽象类。这个答案是不是准确，请老师指点一下。2020-07-29我爱布丁 👍（6） 💬（1）老师举的Filter接口的例子可以用抽象类实现吗？</p><p>我觉得可以说 AuthencationFilter is a Filter, RateLimitFilter is a Filter. 如果两个Filters在doFilter里会预先做一些通用的对于RpcRequest的validation工作，或者有一些shared states，那么这样是不是就应该用抽象类实现呢？</p><p>2019-11-21胡子高兴了 👍（4） 💬（1）老师，接口中可以定义静态成员变量吗？2019-11-20longslee 👍（2） 💬（1）咦，老师，“接口不能包含属性（也就是成员变量）”，是怎么回事？可以的呀2019-12-04小乙哥 👍（2） 💬（1）接口举例的地方，添加实例Filter的地方被注释掉了2019-11-25阿玛铭 👍（2） 💬（2）问句题外话：老师对响应式编程有研究吗？想要一门这样的课程。理由：1.josh long打造，可以代表开发技术的趋势。 2.了解的不够，高并发，类似nety 3.语言隔阂短时间无法突破。 还有一个重要的理由是我觉得老师你讲内容的思路容易理解和接受。2019-11-21DY 👍（1） 💬（2）既然接口能实现的功能抽象类都能实现， 为什么还要用接口？2020-06-10carl 👍（1） 💬（1）Java 中接口可以有属性的吧？2020-04-19刘永超 👍（0） 💬（1）从语法特性区分抽象类和接口是比较实在的，对于语法特性，不能死记，但是理解记忆是必须的。也许对于高手来讲，语法太浅显了，但对于刚入门的人来说，语法是重要的基础。2020-11-14Q罗 👍（0） 💬（1）子类继承抽象类，如果这个子类也是一个抽象类，那这个子类就不是必须实现父类的抽象方法了吧？2020-08-06🍃 👍（0） 💬（2）对于动态编程语言来说，还有一种对接口支持的策略，那就是 duck-typing</p><p>老师, 这个能举个例子吗2020-01-21赵小样 👍（0） 💬（1）老师，使用AndroidStudio开发的时候，抽象类是可以被创建实例的，编译器不会报错，重写抽象类的抽象方法就可以了，抽象类的对象也可以正常调用。那这个是语法问题吧，跟编译器应该没有关系，那说明抽象类是可以被实例化的。能麻烦老师解释一下吗？感谢！2020-01-03言 👍（0） 💬（2）MethodUnSupportedException 这个异常没有找到，protected MockInteface() {} 这个类为啥不能被实例化2019-12-03</p>`,63)])])}const v=n(l,[["render",i]]);export{u as __pageData,v as default};
