...
<span class="line"><span>    lock.unlock();</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>调用线程通过调用get()方法等待RPC返回结果，这个方法里面，你看到的都是熟悉的“面孔”：调用lock()获取锁，在finally里面调用unlock()释放锁；获取锁后，通过经典的在循环中调用await()方法来实现等待。</p><p>当RPC结果返回时，会调用doReceived()方法，这个方法里面，调用lock()获取锁，在finally里面调用unlock()释放锁，获取锁后通过调用signal()来通知调用线程，结果已经返回，不用继续等待了。</p><p>至此，Dubbo里面的异步转同步的源码就分析完了，有没有觉得还挺简单的？最近这几年，工作中需要异步处理的越来越多了，其中有一个主要原因就是有些API本身就是异步API。例如websocket也是一个异步的通信协议，如果基于这个协议实现一个简单的RPC，你也会遇到异步转同步的问题。现在很多公有云的API本身也是异步的，例如创建云主机，就是一个异步的API，调用虽然成功了，但是云主机并没有创建成功，你需要调用另外一个API去轮询云主机的状态。如果你需要在项目内部封装创建云主机的API，你也会面临异步转同步的问题，因为同步的API更易用。</p><h2 id="总结" tabindex="-1">总结 <a class="header-anchor" href="#总结" aria-label="Permalink to &quot;总结&quot;">&amp;ZeroWidthSpace;</a></h2><p>Lock&amp;Condition是管程的一种实现，所以能否用好Lock和Condition要看你对管程模型理解得怎么样。管程的技术前面我们已经专门用了一篇文章做了介绍，你可以结合着来学，理论联系实践，有助于加深理解。</p><p>Lock&amp;Condition实现的管程相对于synchronized实现的管程来说更加灵活、功能也更丰富。</p><p>结合我自己的经验，我认为了解原理比了解实现更能让你快速学好并发编程，所以没有介绍太多Java SDK并发包里锁和条件变量是如何实现的。但如果你对实现感兴趣，可以参考<a href="time://mall?url=https%3A%2F%2Fh5.youzan.com%2Fv2%2Fgoods%2F35z7jjvd4r4oo" target="_blank" rel="noreferrer">《Java并发编程的艺术》</a>一书的第5章《Java中的锁》，里面详细介绍了实现原理，我觉得写得非常好。</p><p>另外，专栏里对DefaultFuture的代码缩减了很多，如果你感兴趣，也可以去看看完整版。<br> Dubbo的源代码在<a href="https://github.com/apache/incubator-dubbo" target="_blank" rel="noreferrer">Github上</a>，DefaultFuture的路径是：incubator-dubbo/dubbo-remoting/dubbo-remoting-api/src/main/java/org/apache/dubbo/remoting/exchange/support/DefaultFuture.java。</p><h2 id="课后思考" tabindex="-1">课后思考 <a class="header-anchor" href="#课后思考" aria-label="Permalink to &quot;课后思考&quot;">&amp;ZeroWidthSpace;</a></h2><p>DefaultFuture里面唤醒等待的线程，用的是signal()，而不是signalAll()，你来分析一下，这样做是否合理呢？</p><p>欢迎在留言区与我分享你的想法，也欢迎你在留言区记录你的思考过程。感谢阅读，如果你觉得这篇文章对你有帮助的话，也欢迎把它分享给更多的朋友。 精选留言（15） ZOU志伟 👍（164） 💬（20）不合理，会导致很多请求超时，看了源码是调用signalAll()2019-04-03张天屹 👍（112） 💬（13）我理解异步的本质是利用多线程提升性能，异步一定是基于一个新开的线程，从调用线程来看是异步的，但是从新开的那个线程来看，正是同步（等待）的，只是对于调用方而言这种同步是透明的。正所谓生活哪有什么岁月静好，只是有人替你负重前行。2019-04-04右耳听海 👍（50） 💬（1）in the method of org.apache.dubbo.remoting.exchange.support.DefaultFuture#doReceived, I think we should call done.signalAll() instead of done.signal() ,and it&#39;s unnecessary to check done != null because it&#39;s always true2019-04-28Geek_e6f3ec 👍（20） 💬（4）老师关于dubbo源码的执行流程有一点疑问。 以下是源码 // 调用通过该方法等待结果 Object get(int timeout){ long start = System.nanoTime(); lock.lock(); try{ while (!isDone()){ done.wait(timeout); // 在这里调用了等待方法后面的代码还能执行吗？ 我理解的管程，是在条件变量等待队列中阻塞等待，被唤醒之后也不是马上执行也要去管程入口等待队列，也就是lock.lock处等待获取锁。 老师是这样的吗？ long cur = System.nanoTime(); if (isDone()||cur-start&gt; timeout){ break; } } }finally { lock.unlock(); } return returnFromResponse();</p><pre><code>}
</code></pre><p>2019-05-15木刻 👍（18） 💬（1）老师今天提到异步转同步，让我想到这两天看的zookeeper客户端源码，感觉应该也是这个机制，客户端同步模式下发送请求后会执行packet.wait，收到服务端响应后执行packet.notifyAll2019-04-02苏格拉底23 👍（14） 💬（2）老师您好！</p><p>有一个基本的问题不明白，如果每个request对应一个线程，似乎并没有用到共享的资源，那么为什么要加锁呢？2019-06-23ban 👍（8） 💬（2）老师，求指教 DefaultFuturewhile这个类为什么要加 while(!isDone()) 这个条件，我看代码while里面加了done.await(timeout);是支持超时的，就是说设置5秒超时， if (isDone() || cur-start &gt; timeout){，只要超过没有被signal()唤醒，那5秒就会自动唤醒，这时候就会在if (isDone() || cur-start &gt; timeout){ 被校验通过，从而break，退出。这时候在加个while条件是不是没必要。 还是说加个while条件是因为时间到点的时候自动唤醒后，Response可能是空，而且时间cur-start &gt; timeout 不超时，所以才有必要进行while再一次判断isDone()是否有值。2019-04-03水目沾 👍（7） 💬（1）这是一对一的关系，肯定只需要 signal。每个线程都是相互独立的，lock 和 condition 也是各自独享的。2019-04-02ycfHH 👍（5） 💬（3）作为一个完全不懂dubbo的新人，我很好奇是什么bug能让signal改成signalAll,因为不管怎么看都感觉signal就已经可以了啊(虽然使用signalall也不错)2019-05-067 👍（5） 💬（1）老师，有个疑问 为什么要判断done!=null呢？这个条件不是永远为true吗。2019-04-03阿甘 👍（3） 💬（2）看了最新的DefaultFuture，已经去掉了lock，老师能分析下最新实现的原理吗2020-01-08遇见阳光 👍（3） 💬（1）老师，我想问下locksupport与此处用lock来阻塞调用者线程有什么区别2019-04-05sibyl 👍（2） 💬（1）动手实现阻塞队列时，一定注意synchronized/wait/notify ， Lock/Condition/await/signal的组合！</p><h2 id="我在实现时-同时用了lock和notify-一直报错illegalmonitorstateexception-该异常表示没加锁-而norify要求的锁必须是synchronized的锁-的确notify必须在synchronized临界区中使用-这是才加synchronized锁的-2020-07-09demon-lee-👍-2-💬-1-tcp-协议本身就是异步的-我们工作中经常用到的-rpc-调用-在-tcp-协议层面-发送完-rpc-请求后-线程是不会等待-rpc-的响应结果的。" tabindex="-1">我在实现时，同时用了Lock和notify，一直报错IllegalMonitorStateException，该异常表示没加锁，而norify要求的锁必须是synchronized的锁，的确notify必须在synchronized临界区中使用，这是才加synchronized锁的！！！2020-07-09Demon.Lee 👍（2） 💬（1）TCP 协议本身就是异步的，我们工作中经常用到的 RPC 调用，在 TCP 协议层面，发送完 RPC 请求后，线程是不会等待 RPC 的响应结果的。 <a class="header-anchor" href="#我在实现时-同时用了lock和notify-一直报错illegalmonitorstateexception-该异常表示没加锁-而norify要求的锁必须是synchronized的锁-的确notify必须在synchronized临界区中使用-这是才加synchronized锁的-2020-07-09demon-lee-👍-2-💬-1-tcp-协议本身就是异步的-我们工作中经常用到的-rpc-调用-在-tcp-协议层面-发送完-rpc-请求后-线程是不会等待-rpc-的响应结果的。" aria-label="Permalink to &quot;我在实现时，同时用了Lock和notify，一直报错IllegalMonitorStateException，该异常表示没加锁，而norify要求的锁必须是synchronized的锁，的确notify必须在synchronized临界区中使用，这是才加synchronized锁的！！！2020-07-09Demon.Lee 👍（2） 💬（1）TCP 协议本身就是异步的，我们工作中经常用到的 RPC 调用，在 TCP 协议层面，发送完 RPC 请求后，线程是不会等待 RPC 的响应结果的。&quot;">&amp;ZeroWidthSpace;</a></h2><p>老师，我也有类似的疑问，如果说TCP都是异步的，那么我们平时用的各种httpClient的sdk开发，它们也做了异步转同步的事情？2019-10-30yc 👍（2） 💬（2）请问老师，阻塞队列的实现，入队和出队都要先获取锁，如果有一个线程正在入队同时又有一个线程在出队，是不是只有一个线程能拿到锁从而成功操作，另一个需要灯unlock，那么入队和出队就是串行了；又或者有两个线程同时入队，也是只有一个线程能够拿到锁从而成功执行入队，另一个线程需要等unlock，也是变成串行了。这样不会影响效率吗？2019-10-15</p>`,42)])])}const b=a(l,[["render",i]]);export{h as __pageData,b as default};
