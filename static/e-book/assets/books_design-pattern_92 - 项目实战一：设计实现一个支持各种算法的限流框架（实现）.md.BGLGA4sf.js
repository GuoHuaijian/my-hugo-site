...
<span class="line"><span>  --RateLimitAlg</span></span></code></pre></div><p>**我们先来看下RateLimiter类。**代码如下所示：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class RateLimiter {</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>RateLimiter类用来串联整个限流流程。它先读取限流规则配置文件，映射为内存中的Java对象（RuleConfig），然后再将这个中间结构构建成一个支持快速查询的数据结构（RateLimitRule）。除此之外，这个类还提供供用户直接使用的最顶层接口（limit()接口）。</p><p>**我们再来看下RuleConfig和ApiLimit两个类。**代码如下所示：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class RuleConfig {</span></span>
...
<span class="line"><span>    limit: 50</span></span>
<span class="line"><span>  - api: /v1/order</span></span>
<span class="line"><span>    limit: 50</span></span></code></pre></div><p><strong>我们再来看下RateLimitRule这个类。</strong></p><p>你可能会好奇，有了RuleConfig来存储限流规则，为什么还要RateLimitRule类呢？这是因为，限流过程中会频繁地查询接口对应的限流规则，为了尽可能地提高查询速度，我们需要将限流规则组织成一种支持按照URL快速查询的数据结构。考虑到URL的重复度比较高，且需要按照前缀来匹配，我们这里选择使用Trie树这种数据结构。我举了个例子解释一下，如下图所示。左边的限流规则对应到Trie树，就是图中右边的样子。</p><p><img src="https://static001.geekbang.org/resource/image/1c/6b/1cf3743dd97fe52ccae5ef62c604976b.jpg?wh=1993%2A889" alt="" loading="lazy" referrerpolicy="no-referrer"></p><p>RateLimitRule的实现代码比较多，我就不在这里贴出来了，我只给出它的定义，如下所示。如果你感兴趣的话，可以自己实现一下，也可以参看我的另一个专栏《数据结构与算法之美》的<a href="https://time.geekbang.org/column/article/80388?utm_term=zeusNGLWQ&amp;utm_source=xiangqingye&amp;utm_medium=geektime&amp;utm_campaign=end&amp;utm_content=xiangqingyelink1104" target="_blank" rel="noreferrer">第55讲</a>。在那节课中，我们对各种接口匹配算法有非常详细的讲解。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class RateLimitRule {</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><h2 id="重点回顾" tabindex="-1">重点回顾 <a class="header-anchor" href="#重点回顾" aria-label="Permalink to &quot;重点回顾&quot;">&amp;ZeroWidthSpace;</a></h2><p>好了，今天的内容到此就讲完了。我们一块来总结回顾一下，你需要重点掌握的内容。</p><p>优秀的代码是重构出来的，复杂的代码是慢慢堆砌出来的。小步快跑、逐步迭代是我比较推崇的开发模式。追求完美主义会让我们迟迟无法下手。所以，为了克服这个问题，一方面，我们可以规划多个小版本来开发，不断迭代优化；另一方面，在编程实现的过程中，我们可以先实现MVP代码，以此来优化重构。</p><p>如何对MVP代码优化重构呢？我们站在Code Reviewer的角度，结合SOLID、DRY、KISS、LOD、基于接口而非实现编程、高内聚松耦合等经典的设计思想和原则，以及编码规范，从代码质量评判标准的角度，来剖析代码在可读性、扩展性、可维护性、灵活、简洁、复用性、可测试性等方面的表现，并且针对性地去优化不足。</p><h2 id="课堂讨论" tabindex="-1">课堂讨论 <a class="header-anchor" href="#课堂讨论" aria-label="Permalink to &quot;课堂讨论&quot;">&amp;ZeroWidthSpace;</a></h2><ol><li>针对MVP代码，如果让你做code review，你还能发现哪些问题？如果让你做重构，你还会做哪些修改和优化？</li><li>如何重构代码，支持自定义限流规则配置文件名和路径？如果你熟悉Java，你可以去了解一下Spring的设计思路，看看如何借鉴到限流框架中来解决这个问题？</li></ol><p>欢迎留言和我分享你的想法。如果有收获，也欢迎你把这篇文章分享给你的朋友。 精选留言（15） Jie 👍（18） 💬（3）https://github.com/wangzheng0822/ratelimiter4j 老师忘记在专栏里面放自己项目的地址了么，翻看隔壁算法之美发现的2020-06-05高源 👍（7） 💬（1）老师今天讲的骨架，有代码吗，我想结合你讲的自己再多考虑和分析，学习其中的方法解决的问题2020-06-03杨杰 👍（3） 💬（1） 1、在配置文件中是否应该指定默认的限流算法和每个api（或appid）对应的算法，在加载配置文件的时候自动生成这个配置算法的实例 2、在RateLimiter中的limit方法里面添加每个api对应的限流算法这个地方感觉有点儿不太好，如果每个API对应的限流算法都不一样会导致大量的If else 判断，是不是应该像第一点说的那样初始化的时候就自动生成了。2020-06-18沧海 👍（1） 💬（1）public class RuleConfig {<br> private List&lt;UniformRuleConfig&gt; configs;<br> public List&lt;AppRuleConfig&gt; getConfigs() {<br> return configs; } } RuleConfig中的属性configs类型应该是List&lt;AppRuleConfig&gt;2020-06-10HuaMax 👍（26） 💬（0）stopwatch.reset()之后要调用stopwatch.start()重新开始，或者stopwatch.stop().start()，亲入坑。。。2020-06-03jaryoung 👍（13） 💬（0）课后习题二： 如何重构代码，支持自定义限流规则配置文件名和路径？</p><pre><code>public static final String DEFAULT_API_LIMIT_CONFIG_NAME = &amp;quot;ratelimiter-rule&amp;quot;;
...
</code></pre><p>Spring boot 如何实现配置文件约定和扫描？可以去看看ConfigFileApplicationListener 这个类，如何跑起来，请去debug，不懂怎么debug，请新建窗口输入 google.com2020-06-03leezer 👍（7） 💬（0）RatelimitAlg在重构后应该是可支持多种算法形式，那么在limit调用的时候应该不是直接new出来，可以通过策略形式进行配置，而算法的选取应该包含默认和指定，也可以配置到文件规则里面。2020-06-03bucher 👍（4） 💬（2）感谢争哥，写的很棒。根据url查找限流规则使用了trie树这块是不是属于过度设计呢？一个app下的api个数不多的情况，直接用map存就可以了吧（map的key使用url名）2021-01-04djfhchdh 👍（4） 💬（0）1、RateLimiter类中，构建api对应在内存中的限流计数器（RateLimitAlg）这个逻辑可以独立出来，初始化的过程中，就将api和相应RateLimitAlg实现类的对应关系建立好； 2、可以使用DI框架，FileRuleConfigSource构建时，从bean配置文件读取构造参数，如果没有提供构造参数就用默认值2020-06-04Heaven 👍（3） 💬（0）1.可以将配置类和实际的拦截器接口实现类进行相分离,然后在实现类里面去执行查找接口拦截规则并执行对应接口的Alg,对于Alg实现类,抽取出接口,方便自定义算法,并且在内部实现诸如漏桶算法的实现,利用用户配置和策略模式来进行实现 2.对于这个问题,可以参考Spring给出的Resource接口,并给出了基于不同的读取方式的实现类,而且为了简化开发,给出ResourceLoader,并且还有着DefaultResourceLoader,可以根据传入前缀,来创建不同的Resource,对于字符串查找树这个实现,我是真的没想到,不过可以在这个基础上,借鉴HashMap的实现,在api接口足够少的时候,使用简单的map保存,多了再转为树 再往深了说,BeanFactory需要传入资源生成对应的实体Bean,而为了简化开发,一般是使用ApplicationContext来初始化Bean,需要传入一个资源给ApplicationContext,并在里面动态解析生成Bean对象,这样的流程,值得我们的框架借鉴点有很多 2020-06-03龙猫 👍（2） 💬（3）tryAcquire 为什么要 调用两次 currentCount.incrementAndGet()方法呢？没太看懂，有大佬解释一下吗2020-10-01Geek_5aae47 👍（2） 💬（0）第二次的updatedCount = currentCount.incrementAndGet()没太看懂放在if外面的用意，是否放入stopwatch.reset()之后会好一点。2020-09-24Tobias 👍（2） 💬（0）课后习题： 让使用者通过注解方式，在项目启动是加载配置。配置可以像这样@Ratelimiter(datasourcetype=&quot;local&quot;, datasourcepath=&#39;xxx&#39;, parser=&#39;json&#39;)2020-08-03Jxin 👍（2） 💬（1）1.随手写都如此牛逼。。。 2.还是有个git代码仓好点，这样手机看难受。 3.为什么要懒加载，直接在初始化时，将算法规则与算法实例绑定，将api与限流算法实例绑定。对于这个限流框架的应用场景不是更合适吗。如此便可以把懒加载的代码抽离，使业务聚焦业务而不用关心实例创建。 4.还得考虑动态限流配置调整的功能。2020-06-03马以 👍（2） 💬（0）哈哈，新鲜出炉2020-06-03</p>`,59)])])}const d=s(e,[["render",l]]);export{m as __pageData,d as default};
