...
<span class="line"><span>  }</span></span>
<span class="line"><span>  //...</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>欢迎留言和我分享你的思考和疑惑。如果有收获，也欢迎你把这篇文章分享给你的朋友。 精选留言（15） zhengyu.nie 👍（259） 💬（30）举个很简单的例子，现在有两个纬度 Car 车 （奔驰、宝马、奥迪等） Transmission 档位类型 （自动挡、手动挡、手自一体等） 按照继承的设计模式，Car是一个Abstract基类，假设有M个车品牌，N个档位一共要写M<em>N个类去描述所有车和档位的结合。 而当我们使用桥接模式的话，我首先new一个具体的Car（如奔驰），再new一个具体的Transmission（比如自动档）。然后奔驰.set(手动档)就可以了。 那么这种模式只有M+N个类就可以描述所有类型，这就是M</em>N的继承类爆炸简化成了M+N组合。</p><p>public abstract class AbstractCar {</p><p>protected Transmission gear;</p><p>public abstract void run();</p><p>public void setTransmission(Transmission gear) { this.gear = gear; }</p><p>}</p><p>所以桥接模式解决的应该是继承爆炸问题。 可以看作是两个abstract组合在一起，独立去拓展，在运行之前将两个具体实现组合到一起。 遵循以下原则 ·依赖倒置原则 ·迪米特法则 ·里氏替换原则 ·接口隔离原则 ·单一职责原则 ·开闭原则</p><p>2020-04-29蹦哒 👍（99） 💬（3）老师请问是否可以这样理解：代理模式是一个类与另一个类的组合，桥接模式是一组类和另外一组类的组合2020-06-13乾坤瞬间 👍（0） 💬（1）回过头再看，总结一下</p><p>桥接在网络osi模型中的链路层上代表一种设备，这种设备通过学习连接到此设备上的计算机macid来识别并原封不动的转发数据包。在软件上抽象层类似于桥接设备，具体实现类似于连接到抽象层的设备一样，如果用户通过抽象的桥接层发送消息，那么就要通过macid寻找具体设备，而这个macid在桥接模式中类似于抽象层定义的需要由具体层实现的方法的集合。 同时也要注意一下物理链路寻找macid的过程是在第一次的时候就动态绑定指定计算机macid到桥接器中，换句话说，可以通过在创建类或者初始化类的时候直接就绑定了对象。这个过程可以通过static方法块和初始化函数中创建2020-11-23风不会停息 。 👍（0） 💬（2）个人理解，这个模式，跟组合模式很像，两者的区别是什么呢？2020-04-30业余爱好者 👍（211） 💬（12）桥接看着就像是面向接口编程这一原则的原旨---将实现与抽象分离。让我迷惑的是，让两者独立变化的说法，接口不是应该稳定吗，为什么要变化?</p><p>多个纬度独立变化那个解释倒是比较容易理解。文中举的警报的例子很贴切。紧急程度和警报的方式可以是两个不同的纬度。可以有不同的组合方式。这与slf4j这一日志门面的设计有异曲同工之妙。slf4j其中有三个核心概念，logger,appender和encoder。分别指这个日志记录器负责哪个类的日志，日志打印到哪里以及日志打印的格式。三个纬度上可以有不同的实现，使用者可以在每一纬度上定义多个实现，配置文件中将各个纬度的某一个实现组合在一起就ok了。</p><p>行文至此，开头的那个问题也有了答案。一句话就是，桥接就是面向接口编程的集大成者。面向接口编程只是说在系统的某一个功能上将接口和实现解藕，而桥接是详细的分析系统功能，将各个独立的纬度都抽象出来，使用时按需组合。2020-03-04下雨天 👍（85） 💬（12）课后题：可以考虑使用建造者模式来重构！参见46讲中</p><p>建造者使用场景： 1.构造方法必填属性很多，需要检验 2.类属性之间有依赖关系或者约束条件 3.创建不可变对象(此题刚好符合这种场景) 2020-02-24松花皮蛋me 👍（33） 💬（16）这个模式和策略模式的区别是？2020-02-24李朝辉 👍（21） 💬（3）一点思考：如果notification类针对一次告警，需要同时在微信、电话、邮件上发送通知，当前的Notification类定义就没办法满足条件了，可以将组合的MsgSender变成一个list或者set，将不同渠道的sender注册进去，这样，就可以在调用notify的时候，将list或set内的sender，都调用一遍send2020-03-06忆水寒 👍（21） 💬（0）参数不多的情况可以在构造函数初始化，如果参数较多 就可以使用建造者模式初始化。2020-02-24攻城拔寨 👍（18） 💬（8）我觉得桥接模式解释成： 一个类存在不同纬度的变化，可以通过组合的方式，让它们独自扩展。 栗子：白色圆形，白色正方形，黑色圆形，黑色正方形。 抽象成 颜色 跟 形状 两个纬度去搞，就是桥接模式啦。 至于 jdbc 的，我水平有限啊，还是理解不了～2020-02-28冰激凌的眼泪 👍（15） 💬（2）在桥接模式中，所谓抽象就是要干什么，所谓实现就是怎么去干，但是这俩是没有抽象与实现的意义的。2020-02-25小晏子 👍（12） 💬（1） “emailAddresses、telephones、wechatIds 中的数据有可能在 Notification 类外部被修改”的原因是对外暴露了修改接口set*，如果不想被修改那么就不要暴露set接口，这样的话初始化这些email，telephone和wechat的工作就放到构造函数里，用构造函数去初始化这些变量，这样初始化之后正常情况下外面没法修改。2020-02-24humor 👍（10） 💬（4）我觉得Notification没有必要再分成三个子类了，直接传入MsgSender子类就可以了吧2020-02-26Geek_pillar6699 👍（8） 💬（1）补充参考： https://refactoringguru.cn/design-patterns/bridge2021-03-08王喜春 👍（7） 💬（1）Notification和MsgSender的场景很形象，不过还是不过瘾。如能基于这个场景， 先IfElse地爆炸实现、实际工作中评估不全、造成线上事故等问题， 再引申出设计大神的桥接模式救场，再复盘地研究下， 桥接模式究竟是哪些天赋异禀造就了神奇的解耦效果， 就更好了。</p><p>不过， 这些内容， 可以留在升级课程中提供一对一的工作坊式教学。</p><p>感谢老师2021-08-06</p>`,52)])])}const h=n(i,[["render",l]]);export{u as __pageData,h as default};
