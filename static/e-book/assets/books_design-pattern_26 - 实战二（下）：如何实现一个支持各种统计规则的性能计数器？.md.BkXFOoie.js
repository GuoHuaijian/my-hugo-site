...
<span class="line"><span>    }</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><h2 id="review设计与实现" tabindex="-1">Review设计与实现 <a class="header-anchor" href="#review设计与实现" aria-label="Permalink to &quot;Review设计与实现&quot;">&amp;ZeroWidthSpace;</a></h2><p>我们前面讲到了SOLID、KISS、DRY、YAGNI、LOD等设计原则，基于接口而非实现编程、多用组合少用继承、高内聚低耦合等设计思想。我们现在就来看下，上面的代码实现是否符合这些设计原则和思想。</p><ul><li>MetricsCollector</li></ul><p>MetricsCollector负责采集和存储数据，职责相对来说还算比较单一。它基于接口而非实现编程，通过依赖注入的方式来传递MetricsStorage对象，可以在不需要修改代码的情况下，灵活地替换不同的存储方式，满足开闭原则。</p><ul><li>MetricsStorage、RedisMetricsStorage</li></ul><p>MetricsStorage和RedisMetricsStorage的设计比较简单。当我们需要实现新的存储方式的时候，只需要实现MetricsStorage接口即可。因为所有用到MetricsStorage和RedisMetricsStorage的地方，都是基于相同的接口函数来编程的，所以，除了在组装类的地方有所改动（从RedisMetricsStorage改为新的存储实现类），其他接口函数调用的地方都不需要改动，满足开闭原则。</p><ul><li>Aggregator</li></ul><p>Aggregator类是一个工具类，里面只有一个静态函数，有50行左右的代码量，负责各种统计数据的计算。当需要扩展新的统计功能的时候，需要修改aggregate()函数代码，并且一旦越来越多的统计功能添加进来之后，这个函数的代码量会持续增加，可读性、可维护性就变差了。所以，从刚刚的分析来看，这个类的设计可能存在职责不够单一、不易扩展等问题，需要在之后的版本中，对其结构做优化。</p><ul><li>ConsoleReporter、EmailReporter</li></ul><p>ConsoleReporter和EmailReporter中存在代码重复问题。在这两个类中，从数据库中取数据、做统计的逻辑都是相同的，可以抽取出来复用，否则就违反了DRY原则。而且整个类负责的事情比较多，职责不是太单一。特别是显示部分的代码，可能会比较复杂（比如Email的展示方式），最好是将显示部分的代码逻辑拆分成独立的类。除此之外，因为代码中涉及线程操作，并且调用了Aggregator的静态函数，所以代码的可测试性不好。</p><p>今天我们给出的代码实现还是有诸多问题的，在后面的章节（第39、40讲）中，我们会慢慢优化，给你展示整个设计演进的过程，这比直接给你最终的最优方案要有意义得多！实际上，优秀的代码都是重构出来的，复杂的代码都是慢慢堆砌出来的 。所以，当你看到那些优秀而复杂的开源代码或者项目代码的时候，也不必自惭形秽，觉得自己写不出来。毕竟罗马不是一天建成的，这些优秀的代码也是靠几年的时间慢慢迭代优化出来的。</p><h2 id="重点回顾" tabindex="-1">重点回顾 <a class="header-anchor" href="#重点回顾" aria-label="Permalink to &quot;重点回顾&quot;">&amp;ZeroWidthSpace;</a></h2><p>好了，今天的内容到此就讲完了。我们一块总结回顾一下，你需要掌握的重点内容。</p><p>写代码的过程本就是一个修修改改、不停调整的过程，肯定不是一气呵成的。你看到的那些大牛开源项目的设计和实现，也都是在不停优化、修改过程中产生的。比如，我们熟悉的Unix系统，第一版很简单、粗糙，代码不到1万行。所以，迭代思维很重要，不要刚开始就追求完美。</p><p>面向对象设计和实现要做的事情，就是把合适的代码放到合适的类中。至于到底选择哪种划分方法，判定的标准是让代码尽量地满足低耦合、高内聚、单一职责、对扩展开放对修改关闭等之前讲的各种设计原则和思想，尽量地做到代码可复用、易读、易扩展、易维护。</p><h2 id="课堂讨论" tabindex="-1">课堂讨论 <a class="header-anchor" href="#课堂讨论" aria-label="Permalink to &quot;课堂讨论&quot;">&amp;ZeroWidthSpace;</a></h2><p>今天课堂讨论题有下面两道。</p><ol><li>对于今天的设计与代码实现，你有没有发现哪些不合理的地方？有哪些可以继续优化的地方呢？或者留言说说你的设计方案。</li><li>说一个你觉得不错的开源框架或者项目，聊聊你为什么觉得它不错？</li></ol><p>欢迎在留言区写下你的答案，和同学一起交流和分享。如果有收获，也欢迎你把这篇文章分享给你的朋友。 精选留言（15） 落叶飞逝的恋 👍（5） 💬（3）老师，请教个问题。类的定义里面，什么时候使用基本类型，什么时候用包装类型？2020-01-04程序员小跃 👍（3） 💬（1）把落下的实战篇学习完了，还把老师的干货总结分享给了自己的小伙伴，都说赞2020-01-06淤白 👍（1） 💬（1）打卡：用Java实现了文中的完整案例。2020-11-23牛顿的烈焰激光剑 👍（0） 💬（2）Demo.java 里应该用<code>while(true);</code>包裹所有代码吧？2020-01-06猫头鹰爱拿铁 👍（0） 💬（2）可否提供下类图，整体上看着更方便。2020-01-02再见孙悟空 👍（0） 💬（5）对于获取大量数据在内存有可能把内存撑爆的问题，有什么好的解法吗？前段时间系统也遇到这个问题了，最后扩大了内存，有没有通过代码上来解决的方案呢？欢迎小伙伴和老师的指教。2020-01-01堵车 👍（63） 💬（8）要写出优美的代码，首先要有一颗对丑陋代码厌恶的心2020-01-02李现教你学编程 👍（53） 💬（9）新年快乐 一起学习 一起提高 20202020-01-01何沛 👍（31） 💬（5）Aggregator考虑到后期新增新的维度统计，可以考虑使用责任链模式。 ConsoleReporter、EmailReporter 出现了代码复用，可以用模板设计模式。2020-01-02辣么大 👍（25） 💬（10）想了三点，希望和小伙伴们讨论一下： 1、RequestInfo save 一次写入一条。是否需要考虑通过设置参数，例如一次写入1000或10000条？好处不用频繁的与数据库建立连接。 2、聚合统计Aggregator是否可以考虑不写代码实现统计的逻辑，而是使用一条SQL查询实现同样的功能？ 3、EmailReporter startDailyReport 没指定明确的统计起止时间。设置统计指定区间的request info，例如08:00~次日08:00，然后发邮件。2020-01-01Jxin 👍（18） 💬（4）沙发！ 1.栏主新年快乐。零点发帖，啧啧啧。 2.给出github地址吧，我们来提pr，一个学习用demo大家合力下就当练手，没必要自己死磕全实现哈。 3.关于邮件和控制台两个接入层。实现代码重了。可以把定时统计下沉到下一层来实现，然后两个接入层共用这个实现。然后收集的统计数据的类型应该可以提供差异化配置的api。在消费统计数据的消息时，做差异化分发，实现各接入层仅看到自己想看的数据。</p><p>4.spring1.x~3.x，兼容老版本做得挺好。springboot在自动装配的实现上下足了功夫（插件化，易插拔）。netty的实现也挺挺讲究，还能顺带学网络相关知识。以上其实都运用一系列设计原则。在没看栏主专栏前，我是啃这些学的场景。2020-01-01RedDevil 👍（17） 💬（0）争哥一直在强调迭代思维，我也是最近才意识到这个，以前做事总认为第一遍就能搞定，不愿意去搞第二次甚至第N次，实际上反复迭代优化后的成果才更有深度2020-10-13卫江 👍（15） 💬（3）上面的代码设计与实现，我认为有两个重点是需要改进的：</p><ol><li>不同的统计规则，通过抽象统计规则抽象类，每一个具体的统计（最大时间，平均时间）单独实现，同时在 Aggregator 内中通过 List等容器保存所有的统计规则实现类，提供注册函数来动态添加新的统计规则，使得Aggregator否则开闭原则，各个统计规则也符合单一责任原则。</li><li>显示方式很明显是一个变化点，需要抽象封装，抽象出 显示接口，在汇报类中通过依赖注入的方式来使用具体的显示类，这样一来，reporter类更加责任单一，我们也可以通过扩展新的显示类来扩展功能，符合开闭原则，每一个显示实现类更加否则单一责任。2020-01-02郑大钱 👍（11） 💬（1）哈哈，原来谷歌的工程师也是普通人，也不可能一上来就写出完美的代码。小步快跑、逐步迭代不仅是一种很好的产品模式，也是一种很好的开发模式，那些著名的开源库也是修修补补逐渐长大的。 最小原型也是一种给外界释放产出信号的有效手段，需求方的情绪得到安抚，写代码的时候自己也不会那么慌。2020-09-11Murrre 👍（11） 💬（0）https://github.com/murreIsCoding/learning_geek/tree/master/src/main/java/design_pattern/demo2/performance_monitoring 敲了一下，主要是实现了redis存储部分逻辑，redis命令不是很熟，可能有更好的方案2020-01-02</li></ol>`,54)])])}const m=n(l,[["render",i]]);export{d as __pageData,m as default};
