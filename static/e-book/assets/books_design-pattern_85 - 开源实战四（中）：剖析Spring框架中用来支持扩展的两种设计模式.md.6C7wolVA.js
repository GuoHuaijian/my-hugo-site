import{_ as s,o as a,c as p,ae as e}from"./chunks/framework.Iv6F95cJ.js";const g=JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"books/design-pattern/85 - 开源实战四（中）：剖析Spring框架中用来支持扩展的两种设计模式.md","filePath":"books/design-pattern/85 - 开源实战四（中）：剖析Spring框架中用来支持扩展的两种设计模式.md"}'),i={name:"books/design-pattern/85 - 开源实战四（中）：剖析Spring框架中用来支持扩展的两种设计模式.md"};function t(l,n,c,o,r,v){return a(),p("div",null,[...n[0]||(n[0]=[e(`<p>上一节课中，我们学习了Spring框架背后蕴藏的一些经典设计思想，比如约定优于配置、低侵入松耦合、模块化轻量级等等。我们可以将这些设计思想借鉴到其他框架开发中，在大的设计层面提高框架的代码质量。这也是我们在专栏中讲解这部分内容的原因。</p><p>除了上一节课中讲到的设计思想，实际上，可扩展也是大部分框架应该具备的一个重要特性。所谓的框架可扩展，我们之前也提到过，意思就是，框架使用者在不修改框架源码的情况下，基于扩展点定制扩展新的功能。</p><p>前面在理论部分，我们也讲到，常用来实现扩展特性的设计模式有：观察者模式、模板模式、职责链模式、策略模式等。今天，我们再剖析Spring框架为了支持可扩展特性用的2种设计模式：观察者模式和模板模式。</p><p>话不多说，让我们正式开始今天的学习吧！</p><h2 id="观察者模式在spring中的应用" tabindex="-1">观察者模式在Spring中的应用 <a class="header-anchor" href="#观察者模式在spring中的应用" aria-label="Permalink to &quot;观察者模式在Spring中的应用&quot;">&amp;ZeroWidthSpace;</a></h2><p>在前面我们讲到，Java、Google Guava都提供了观察者模式的实现框架。Java提供的框架比较简单，只包含java.util.Observable和java.util.Observer两个类。Google Guava提供的框架功能比较完善和强大：通过EventBus事件总线来实现观察者模式。实际上，Spring也提供了观察者模式的实现框架。今天，我们就再来讲一讲它。</p><p>Spring中实现的观察者模式包含三部分：Event事件（相当于消息）、Listener监听者（相当于观察者）、Publisher发送者（相当于被观察者）。我们通过一个例子来看下，Spring提供的观察者模式是怎么使用的。代码如下所示：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>// Event事件</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p>我们再来看下，如何通过BeanPostProcessor来定义初始化前置和后置操作？</p><p>我们只需要定义一个实现了BeanPostProcessor接口的处理器类，并在配置文件中像配置普通Bean一样去配置就可以了。Spring中的ApplicationContext会自动检测在配置文件中实现了BeanPostProcessor接口的所有Bean，并把它们注册到BeanPostProcessor处理器列表中。在Spring容器创建Bean的过程中，Spring会逐一去调用这些处理器。</p><p>通过上面的分析，我们基本上弄清楚了Spring Bean的整个生命周期（创建加销毁）。针对这个过程，我画了一张图，你可以结合着刚刚讲解一块看下。</p><p><img src="https://static001.geekbang.org/resource/image/ca/4d/cacaf86b03a9432a4885385d2869264d.jpg?wh=2693%2A1352" alt="" loading="lazy" referrerpolicy="no-referrer"></p><p>不过，你可能会说，这里哪里用到了模板模式啊？模板模式不是需要定义一个包含模板方法的抽象模板类，以及定义子类实现模板方法吗？</p><p>实际上，这里的模板模式的实现，并不是标准的抽象类的实现方式，而是有点类似我们前面讲到的Callback回调的实现方式，也就是将要执行的函数封装成对象（比如，初始化方法封装成InitializingBean对象），传递给模板（BeanFactory）来执行。</p><h2 id="重点回顾" tabindex="-1">重点回顾 <a class="header-anchor" href="#重点回顾" aria-label="Permalink to &quot;重点回顾&quot;">&amp;ZeroWidthSpace;</a></h2><p>好了，今天的内容到此就讲完了。我们一块来总结回顾一下，你需要重点掌握的内容。</p><p>今天我讲到了Spring中用到的两种支持扩展的设计模式，观察者模式和模板模式。</p><p>其中，观察者模式在Java、Google Guava、Spring中都有提供相应的实现代码。在平时的项目开发中，基于这些实现代码，我们可以轻松地实现一个观察者模式。</p><p>Java提供的框架比较简单，只包含java.util.Observable和java.util.Observer两个类。Google Guava提供的框架功能比较完善和强大，可以通过EventBus事件总线来实现观察者模式。Spring提供了观察者模式包含Event事件、Listener监听者、Publisher发送者三部分。事件发送到ApplicationContext中，然后，ApplicationConext将消息发送给事先注册好的监听者。</p><p>除此之外，我们还讲到模板模式在Spring中的一个典型应用，那就是Bean的创建过程。Bean的创建包含两个大的步骤，对象的创建和对象的初始化。其中，对象的初始化又可以分解为3个小的步骤：初始化前置操作、初始化、初始化后置操作。</p><h2 id="课堂讨论" tabindex="-1">课堂讨论 <a class="header-anchor" href="#课堂讨论" aria-label="Permalink to &quot;课堂讨论&quot;">&amp;ZeroWidthSpace;</a></h2><p>在Google Guava的EventBus实现中，被观察者发送消息到事件总线，事件总线根据消息的类型，将消息发送给可匹配的观察者。那在Spring提供的观察者模式的实现中，是否也支持按照消息类型匹配观察者呢？如果能，它是如何实现的？如果不能，你有什么方法可以让它支持吗？</p><p>欢迎留言和我分享你的想法。如果有收获，也欢迎你把这篇文章分享给你的朋友。 精选留言（14） zz 👍（36） 💬（7）看到源码中有这么多的if else，瞬间给了自己一些信心。2020-06-05悟光 👍（11） 💬（0）支持按照消息类型匹配观察者，最终调用 SimpleApplicationEventMulticaster 类的multicastEvent方法通过反射匹配类型。根据配置采用异步还是同步的监听方式。 public void multicastEvent(final ApplicationEvent event, @Nullable ResolvableType eventType) { ResolvableType type = (eventType != null ? eventType : resolveDefaultEventType(event)); Executor executor = getTaskExecutor(); for (ApplicationListener&lt;?&gt; listener : getApplicationListeners(event, type)) { if (executor != null) { executor.execute(() -&gt; invokeListener(listener, event)); } else { invokeListener(listener, event); } } } 2020-05-18Heaven 👍（10） 💬（1）看了下源码,其流程可以从 图片: https://uploader.shimo.im/f/fZuIVWFIlWQnnRFq.png 推送Event时候,去发送Event开始走 主要就是这个 在此方法中,会调用getApplicationListeners(event,eventType)函数 图片: https://uploader.shimo.im/f/3mZZvSBhmc8CXLnx.png 在这个方法中,会获取到对应的所有监听者,如何获取到的,会先通过一个锁来从一个名为retrieverCache的map中尝试获取到对应的监听者 如果拿不到,会进入到retrieveApplicationListeners()这个函数之中 图片: https://uploader.shimo.im/f/GFvS2QEKGlMctZrc.png 在这个方法中,会在add返回的结果的时候,会调用一个方法supportsEvent(), 这才是真正进行匹配的方法 图片: https://uploader.shimo.im/f/102Ia9ToqIw5ZOyq.png 匹配事件和源类型是否一致,一致才算做可以发送2020-05-18test 👍（8） 💬（0）用反射获取的type2020-05-18Monday 👍（7） 💬（1）定义了一个bean同时实现了InitializingBean, BeanPostProcessor, DisposableBean，发现方法跟老师最后一张图的不一致： 1、顺序是构造器、afterPropertiesSet、postProcessBeforeInitialization、postProcessAfterInitialization、destroy 2、postProcessBeforeInitialization、postProcessAfterInitialization这两个方法交替执行了N次2020-05-20松小鼠 👍（6） 💬（3）昨天刚好在隔壁小马哥那里看到了，两个课一起听，侧重点不同，都很重要啊2020-05-18Tobias 👍（3） 💬（0）Spring 提供的观察者模式是支持按照消息类型匹配观察者。getApplicationListeners(event, type) 方法会根据eventtype 找到对应的的listeners. getApplicationListeners(event, type) 通过反射找到 event 以及event的子类 对应的listeners.2020-07-27tonyli 👍（2） 💬（0）使用模板模式定义了一系列步骤的骨架，是各类框架的根本设计模式。2022-09-19握了个大蚂蚱 👍（2） 💬（0）1.实现InitializingBean的初始化方法，也是约定优于配置的一个体现，只不过不是覆盖默认值而是实现init-method的一个前置方法afterPropertiesSet。 2.实现InitializingBean的初始化方法和自己指定init-method相比，侵入性更高，所以不太推荐。可以用注解版的@Bean(initMethod = &quot;xx&quot;)来指定初始化方法，或者使用JSR250中的@PostConstruct标注在初始化方法上来让程序回调。2020-10-12剑八 👍（2） 💬（1）spring中的refresh是一个模板方法： 大致有：注册beanFactoryPostProcessor，beanPostProcessor，读取bean definition，创建并初始化bean,等2020-07-05李金鹏 👍（0） 💬（0）自定义注解加AOP可以实现支持按照消息类型匹配观察者2022-01-24鹤涵 👍（0） 💬（1）策论模式 缓存对象类型和处理器的关系2021-02-25子房 👍（0） 💬（0）spring 中通过 simple 实现类发送事件，并维护了事件和监听者的映射关系，当需要发送一个事件时会把该事件对应的监听者取出，并执行。2021-02-04o0oi1i 👍（0） 💬（0）打卡852020-11-02</p>`,47)])])}const d=s(i,[["render",t]]);export{g as __pageData,d as default};
