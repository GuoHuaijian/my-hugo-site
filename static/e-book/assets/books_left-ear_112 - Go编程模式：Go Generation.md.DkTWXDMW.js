import{_ as s,o as a,c as p,ae as e}from"./chunks/framework.Iv6F95cJ.js";const h=JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"books/left-ear/112 - Go编程模式：Go Generation.md","filePath":"books/left-ear/112 - Go编程模式：Go Generation.md"}'),l={name:"books/left-ear/112 - Go编程模式：Go Generation.md"};function i(t,n,c,o,r,d){return a(),p("div",null,[...n[0]||(n[0]=[e(`<p>你好，我是陈皓，网名左耳朵耗子。</p><p>这节课，我们来学习一下Go语言的代码生成的玩法。</p><p>Go语言的代码生成主要还是用来解决编程泛型的问题。泛型编程主要是解决这样一个问题：因为静态类型语言有类型，所以，相关的算法或是对数据处理的程序会因为类型不同而需要复制一份，这样会导致数据类型和算法功能耦合。</p><p>我之所以说泛型编程可以解决这样的问题，就是说，在写代码的时候，不用关心处理数据的类型，只需要关心相关的处理逻辑。</p><p>泛型编程是静态语言中非常非常重要的特征，如果没有泛型，我们就很难做到多态，也很难完成抽象，这就会导致我们的代码冗余量很大。</p><h2 id="现实中的类比" tabindex="-1">现实中的类比 <a class="header-anchor" href="#现实中的类比" aria-label="Permalink to &quot;现实中的类比&quot;">&amp;ZeroWidthSpace;</a></h2><p>为了帮你更好地理解，我举个现实当中的例子。我们用螺丝刀来做打比方，螺丝刀本来只有一个拧螺丝的作用，但是因为螺丝的类型太多，有平口的，有十字口的，有六角的……螺丝还有不同的尺寸，这就导致我们的螺丝刀为了要适配各种千奇百怪的螺丝类型（样式和尺寸），也是各种样式的。</p><p>而真正的抽象是，螺丝刀不应该关心螺丝的类型，它只要关注自己的功能是不是完备，并且让自己可以适配不同类型的螺丝就行了，这就是所谓的泛型编程要解决的实际问题。</p><h2 id="go语方的类型检查" tabindex="-1">Go语方的类型检查 <a class="header-anchor" href="#go语方的类型检查" aria-label="Permalink to &quot;Go语方的类型检查&quot;">&amp;ZeroWidthSpace;</a></h2><p>因为Go语言目前并不支持真正的泛型，所以，只能用 <code>interface{}</code> 这样的类似于 <code>void*</code> 的过度泛型来玩，这就导致我们要在实际过程中进行类型检查。</p><p>Go语言的类型检查有两种技术，一种是 Type Assert，一种是Reflection。</p><h3 id="type-assert" tabindex="-1">Type Assert <a class="header-anchor" href="#type-assert" aria-label="Permalink to &quot;Type Assert&quot;">&amp;ZeroWidthSpace;</a></h3><p>这种技术，一般是对某个变量进行 <code>.(type)</code>的转型操作，它会返回两个值，分别是variable和error。 variable是被转换好的类型，error表示如果不能转换类型，则会报错。</p><p>在下面的示例中，我们有一个通用类型的容器，可以进行 <code>Put(val)</code>和 <code>Get()</code>，注意，这里使用了 <code>interface{}</code>做泛型。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>//Container is a generic container, accepting anything.</span></span>
...
<span class="line"><span>}</span></span>
<span class="line"><span></span></span>
<span class="line"><span>fmt.Printf(&quot;assertExample: %d (%T)\\n&quot;, elem, elem)</span></span></code></pre></div><h3 id="reflection" tabindex="-1">Reflection <a class="header-anchor" href="#reflection" aria-label="Permalink to &quot;Reflection&quot;">&amp;ZeroWidthSpace;</a></h3><p>对于Reflection，我们需要把上面的代码修改如下：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>type Container struct {</span></span>
...
<span class="line"><span>fmt.Println(c.s.Index(0)) //1.4142135623</span></span></code></pre></div><p>可以看到，Type Assert是不用了，但是用反射写出来的代码还是有点复杂的。那么，有没有什么好的方法？</p><h2 id="他山之石" tabindex="-1">他山之石 <a class="header-anchor" href="#他山之石" aria-label="Permalink to &quot;他山之石&quot;">&amp;ZeroWidthSpace;</a></h2><p>对于泛型编程最牛的语言 C++ 来说，这类问题都是使用 Template解决的。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>//用&lt;class T&gt;来描述泛型</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p>可以看到，函数模板中我们有如下的占位符：</p><ul><li><code>PACKAGE_NAME</code>：包名</li><li><code>GENERIC_NAME</code> ：名字</li><li><code>GENERIC_TYPE</code> ：实际的类型</li></ul><p>其它的代码都是一样的。</p><h3 id="函数生成脚本" tabindex="-1">函数生成脚本 <a class="header-anchor" href="#函数生成脚本" aria-label="Permalink to &quot;函数生成脚本&quot;">&amp;ZeroWidthSpace;</a></h3><p>然后，我们有一个叫<code>gen.sh</code>的生成脚本，如下所示：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>#!/bin/bash</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>    c.s = c.s[1:]</span></span>
<span class="line"><span>    return r</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>这两份代码可以让我们的代码完全编译通过，付出的代价就是需要多执行一步 <code>go generate</code> 命令。</p><h2 id="新版filter" tabindex="-1">新版Filter <a class="header-anchor" href="#新版filter" aria-label="Permalink to &quot;新版Filter&quot;">&amp;ZeroWidthSpace;</a></h2><p>现在我们再回头看看上节课里的那些用反射整出来的例子，你就会发现，有了这样的技术，我们就不用在代码里，用那些晦涩难懂的反射来做运行时的类型检查了。我们可以写出很干净的代码，让编译器在编译时检查类型对不对。</p><p>下面是一个Fitler的模板文件 <code>filter.tmp.go</code>：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>package PACKAGE_NAME</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>    fmt.Println(e)</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><h2 id="第三方工具" tabindex="-1">第三方工具 <a class="header-anchor" href="#第三方工具" aria-label="Permalink to &quot;第三方工具&quot;">&amp;ZeroWidthSpace;</a></h2><p>我们并不需要自己手写 <code>gen.sh</code> 这样的工具类，我们可以直接使用第三方已经写好的工具。我给你提供一个列表。</p><ul><li><a href="https://github.com/cheekybits/genny" target="_blank" rel="noreferrer">Genny</a></li><li><a href="https://github.com/taylorchu/generic" target="_blank" rel="noreferrer">Generic</a></li><li><a href="https://github.com/joeshaw/gengen" target="_blank" rel="noreferrer">GenGen</a></li><li><a href="https://github.com/clipperhouse/gen" target="_blank" rel="noreferrer">Gen</a></li></ul><p>好了，这节课就到这里。如果你觉得今天的内容对你有所帮助，欢迎你帮我分享给更多人。 精选留言（5） 萧 👍（14） 💬（0）他山之石可以攻玉，语言只是工具，那些编程思想才是真正重要的，皓叔强2021-02-18林万程 👍（6） 💬（0）现在有泛型了2022-04-21Steven 👍（1） 💬（0）这个玩法，有点牛逼了，开拓视野啊。感觉很nice，写一个模版，如果模版好，后期根据模版直接批量生产业务代码，简单、干净而且还可以语义化。配置生产代码的方式写代码。2022-10-17顺势而为 👍（0） 💬（0）低代码的背后，应该是这些脚本工具吧？2022-08-16方勇(gopher) 👍（0） 💬（0）代码生成器，目前在做微服务troubleshooting，策略配置基于此模式！2021-12-23</p>`,71)])])}const g=s(l,[["render",i]]);export{h as __pageData,g as default};
