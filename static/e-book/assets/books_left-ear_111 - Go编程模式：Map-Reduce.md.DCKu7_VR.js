import{_ as s,o as a,c as p,ae as e}from"./chunks/framework.Iv6F95cJ.js";const f=JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"books/left-ear/111 - Go编程模式：Map-Reduce.md","filePath":"books/left-ear/111 - Go编程模式：Map-Reduce.md"}'),l={name:"books/left-ear/111 - Go编程模式：Map-Reduce.md"};function i(t,n,c,o,r,u){return a(),p("div",null,[...n[0]||(n[0]=[e(`<p>你好，我是陈皓，网名左耳朵耗子。</p><p>这节课，我们来学习一下函数式编程中非常重要的Map、Reduce、Filter这三种操作。这三种操作可以让我们轻松灵活地进行一些数据处理，毕竟，我们的程序大多数情况下都在倒腾数据。尤其是对于一些需要统计的业务场景来说，Map、Reduce、Filter是非常通用的玩法。</p><p>话不多说，我们先来看几个例子。</p><h2 id="基本示例" tabindex="-1">基本示例 <a class="header-anchor" href="#基本示例" aria-label="Permalink to &quot;基本示例&quot;">&amp;ZeroWidthSpace;</a></h2><h3 id="map示例" tabindex="-1">Map示例 <a class="header-anchor" href="#map示例" aria-label="Permalink to &quot;Map示例&quot;">&amp;ZeroWidthSpace;</a></h3><p>在下面的程序代码中，我写了两个Map函数，这两个函数需要两个参数：</p><ul><li>一个是字符串数组 <code>[]</code> <code>string</code>，说明需要处理的数据是一个字符串；</li><li>另一个是一个函数func(s string) string 或 func(s string) int。</li></ul><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>func MapStrToStr(arr []string, fn func(s string) string) []string {</span></span>
...
<span class="line"><span>})</span></span>
<span class="line"><span>fmt.Printf(&quot;%v\\n&quot;, y)</span></span>
<span class="line"><span>//[3, 4, 8]</span></span></code></pre></div><p>可以看到，我们给第一个 <code>MapStrToStr()</code> 传了功能为“转大写”的函数，于是出来的数组就成了全大写的，给<code>MapStrToInt()</code> 传的是计算长度，所以出来的数组是每个字符串的长度。</p><p>我们再来看一下Reduce和Filter的函数是什么样的。</p><h3 id="reduce-示例" tabindex="-1">Reduce 示例 <a class="header-anchor" href="#reduce-示例" aria-label="Permalink to &quot;Reduce 示例&quot;">&amp;ZeroWidthSpace;</a></h3><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>func Reduce(arr []string, fn func(s string) int) int {</span></span>
...
<span class="line"><span>})</span></span>
<span class="line"><span>fmt.Printf(&quot;%v\\n&quot;, x)</span></span>
<span class="line"><span>// 15</span></span></code></pre></div><h3 id="filter示例" tabindex="-1">Filter示例 <a class="header-anchor" href="#filter示例" aria-label="Permalink to &quot;Filter示例&quot;">&amp;ZeroWidthSpace;</a></h3><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>func Filter(arr []int, fn func(n int) bool) []int {</span></span>
<span class="line"><span>    var newArray = []int{}</span></span>
<span class="line"><span>    for _, it := range arr {</span></span>
...
<span class="line"><span>})</span></span>
<span class="line"><span>fmt.Printf(&quot;%v\\n&quot;, out)</span></span></code></pre></div><p>为了方便你理解呢，我给你展示一张图，它形象地说明了Map-Reduce的业务语义，在数据处理中非常有用。</p><p><img src="https://static001.geekbang.org/resource/image/15/56/1506b63044071bfa5c214a725a9caf56.png?wh=794%2A442" alt="" loading="lazy" referrerpolicy="no-referrer"></p><h2 id="业务示例" tabindex="-1">业务示例 <a class="header-anchor" href="#业务示例" aria-label="Permalink to &quot;业务示例&quot;">&amp;ZeroWidthSpace;</a></h2><p>通过刚刚的一些示例，你现在应该有点明白了，Map、Reduce、Filter只是一种控制逻辑，真正的业务逻辑是以传给它们的数据和函数来定义的。</p><p>是的，这是一个很经典的“业务逻辑”和“控制逻辑”分离解耦的编程模式。</p><p>接下来，我们来看一个有业务意义的代码，来进一步帮助你理解什么叫“控制逻辑”与“业务逻辑”分离。</p><h3 id="员工信息" tabindex="-1">员工信息 <a class="header-anchor" href="#员工信息" aria-label="Permalink to &quot;员工信息&quot;">&amp;ZeroWidthSpace;</a></h3><p>首先，我们有一个员工对象和一些数据：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>type Employee struct {</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><h3 id="相关的reduce、fitler函数" tabindex="-1">相关的Reduce、Fitler函数 <a class="header-anchor" href="#相关的reduce、fitler函数" aria-label="Permalink to &quot;相关的Reduce、Fitler函数&quot;">&amp;ZeroWidthSpace;</a></h3><p>然后，我们有下面的几个函数：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>func EmployeeCountIf(list []Employee, fn func(e *Employee) bool) int {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>    return sum</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>简单说明一下：</p><ul><li><code>EmployeeConutIf</code> 和 <code>EmployeeSumIf</code> 分别用于统计满足某个条件的个数或总数。它们都是Filter + Reduce的语义。</li><li><code>EmployeeFilterIn</code> 就是按某种条件过滤，就是Fitler的语义。</li></ul><h3 id="各种自定义的统计示例" tabindex="-1">各种自定义的统计示例 <a class="header-anchor" href="#各种自定义的统计示例" aria-label="Permalink to &quot;各种自定义的统计示例&quot;">&amp;ZeroWidthSpace;</a></h3><p>于是，我们就可以有接下来的代码了。</p><p><strong>1.统计有多少员工大于40岁</strong></p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>old := EmployeeCountIf(list, func(e *Employee) bool {</span></span>
...
<span class="line"><span>})</span></span></code></pre></div><h2 id="泛型map-reduce" tabindex="-1">泛型Map-Reduce <a class="header-anchor" href="#泛型map-reduce" aria-label="Permalink to &quot;泛型Map-Reduce&quot;">&amp;ZeroWidthSpace;</a></h2><p>刚刚的Map-Reduce都因为要处理数据的类型不同，而需要写出不同版本的Map-Reduce，虽然它们的代码看上去是很类似的。所以，这里就要提到泛型编程了。</p><h3 id="简单版-generic-map" tabindex="-1">简单版 Generic Map <a class="header-anchor" href="#简单版-generic-map" aria-label="Permalink to &quot;简单版 Generic Map&quot;">&amp;ZeroWidthSpace;</a></h3><p>我在写这节课的时候，Go语言还不支持泛型（注：Go开发团队技术负责人Russ Cox在2012年11月21golang-dev上的mail确认了Go泛型将在Go 1.18版本落地，时间是2022年2月）。所以，目前的Go语言的泛型只能用 <code>interface{}</code> + <code>reflect</code>来完成。<code>interface{}</code> 可以理解为C中的 <code>void*</code>、Java中的 <code>Object</code> ，<code>reflect</code>是Go的反射机制包，作用是在运行时检查类型。</p><p>下面，我们来看一下，一个非常简单的、不做任何类型检查的泛型的Map函数怎么写。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>func Map(data interface{}, fn interface{}) []interface{} {</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><h3 id="健壮版的-generic-filter" tabindex="-1">健壮版的 Generic Filter <a class="header-anchor" href="#健壮版的-generic-filter" aria-label="Permalink to &quot;健壮版的 Generic Filter&quot;">&amp;ZeroWidthSpace;</a></h3><p>同样，泛型版的 Filter 代码如下（同样分是否“就地计算”的两个版本）：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>func Filter(slice, function interface{}) interface{} {</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><h2 id="后记" tabindex="-1">后记 <a class="header-anchor" href="#后记" aria-label="Permalink to &quot;后记&quot;">&amp;ZeroWidthSpace;</a></h2><p>最后，还有几个未尽事宜：</p><ol><li>使用反射来做这些东西会有一个问题，<strong>那就是代码的性能会很差。所以，上面的代码不能用在需要高性能的地方</strong>。怎么解决这个问题，我会在下节课给你介绍下。</li><li>这节课中的代码大量地参考了 Rob Pike的版本，你可以点击这个链接查看： <a href="https://github.com/robpike/filter" target="_blank" rel="noreferrer">https://github.com/robpike/filter</a>。</li><li>其实，在全世界范围内，有大量的程序员都在问Go语言官方什么时候在标准库中支持 Map、Reduce。Rob Pike说，这种东西难写吗？还要我们官方来帮你们写吗？这种代码我多少年前就写过了，但是，我一次都没有用过，我还是喜欢用“For循环”，我觉得你最好也跟我一起用 “For循环”。</li></ol><p>我个人觉得，Map、Reduce在数据处理的时候还是很有用的，Rob Pike可能平时也不怎么写“业务逻辑”的代码，所以，他可能也不太了解业务的变化有多么频繁……</p><p>当然，好还是不好，由你来判断，但多学一些编程模式，一定是对自己很有帮助的。</p><p>好了，这节课就到这里。如果你觉得今天的内容对你有所帮助，欢迎你帮我分享给更多人。 精选留言（4） 北国骑士 👍（11） 💬（1）所以我觉得用go写业务逻辑真的是件很sb的事情2021-04-16拉欧 👍（2） 💬（1）就是因为go最初不支持泛型，所以每个go框架都要搞一堆的代码生成器来生成重复代码，恶心的不要不要的2022-04-30萧 👍（1） 💬（0）通俗易懂2021-02-18Refrain 👍（0） 💬（1）verifyFuncSignature(fn, elemType, nil) 为什么要传一个空值呢？</p><p>outType := types[len(types)-1] 这个地方outType必然是空值 2021-02-05</p>`,84)])])}const h=s(l,[["render",i]]);export{f as __pageData,h as default};
