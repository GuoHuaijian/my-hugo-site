import{_ as s,o as a,c as p,ae as e}from"./chunks/framework.Iv6F95cJ.js";const b=JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"books/left-ear/110 - Go编程模式：委托和反转控制.md","filePath":"books/left-ear/110 - Go编程模式：委托和反转控制.md"}'),t={name:"books/left-ear/110 - Go编程模式：委托和反转控制.md"};function l(i,n,c,o,d,r){return a(),p("div",null,[...n[0]||(n[0]=[e(`<p>你好，我是陈皓，网名左耳朵耗子。</p><p>控制反转（<a href="https://en.wikipedia.org/wiki/Inversion_of_control" target="_blank" rel="noreferrer">Inversion of Control</a><a href="https://en.wikipedia.org/wiki/Inversion_of_control" target="_blank" rel="noreferrer">，loC</a> ）是一种软件设计的方法，它的主要思想是把控制逻辑与业务逻辑分开，不要在业务逻辑里写控制逻辑，因为这样会让控制逻辑依赖于业务逻辑，而是反过来，让业务逻辑依赖控制逻辑。</p><p>我之前在《<a href="https://coolshell.cn/articles/9949.html" target="_blank" rel="noreferrer">IoC/DIP其实是一种管理思想</a>》这篇文章中，举过一个开关和电灯的例子。其实，这里的开关就是控制逻辑，电器是业务逻辑。我们不要在电器中实现开关，而是要把开关抽象成一种协议，让电器都依赖它。这样的编程方式可以有效降低程序复杂度，并提升代码重用度。</p><p>面向对象的设计模式我就不提了，我们来看看Go语言使用Embed结构的一个示例。</p><h2 id="嵌入和委托" tabindex="-1">嵌入和委托 <a class="header-anchor" href="#嵌入和委托" aria-label="Permalink to &quot;嵌入和委托&quot;">&amp;ZeroWidthSpace;</a></h2><h3 id="结构体嵌入" tabindex="-1">结构体嵌入 <a class="header-anchor" href="#结构体嵌入" aria-label="Permalink to &quot;结构体嵌入&quot;">&amp;ZeroWidthSpace;</a></h3><p>在Go语言中，我们可以很轻松地把一个结构体嵌到另一个结构体中，如下所示：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>type Widget struct {</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><h3 id="方法重写" tabindex="-1">方法重写 <a class="header-anchor" href="#方法重写" aria-label="Permalink to &quot;方法重写&quot;">&amp;ZeroWidthSpace;</a></h3><p>然后，我们需要两个接口：用Painter把组件画出来；Clicker 用于表明点击事件。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>type Painter interface {</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p>说到这儿，我要重点提醒你一下，<code>Button.Paint()</code> 接口可以通过 Label 的嵌入带到新的结构体，如果 <code>Button.Paint()</code> 不实现的话，会调用 <code>Label.Paint()</code> ，所以，在 <code>Button</code> 中声明 <code>Paint()</code> 方法，相当于Override。</p><h3 id="嵌入结构多态" tabindex="-1">嵌入结构多态 <a class="header-anchor" href="#嵌入结构多态" aria-label="Permalink to &quot;嵌入结构多态&quot;">&amp;ZeroWidthSpace;</a></h3><p>从下面的程序中，我们可以看到整个多态是怎么执行的。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>button1 := Button{Label{Widget{10, 70}, &quot;OK&quot;}}</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p>我们可以使用接口来多态，也可以使用泛型的 <code>interface{}</code> 来多态，但是需要有一个类型转换。</p><h2 id="反转控制" tabindex="-1">反转控制 <a class="header-anchor" href="#反转控制" aria-label="Permalink to &quot;反转控制&quot;">&amp;ZeroWidthSpace;</a></h2><p>我们再来看一个示例。</p><p>我们有一个存放整数的数据结构，如下所示：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>type IntSet struct {</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p>我来解释下这段代码。</p><ul><li>我们在 <code>UndoableIntSet</code> 中嵌入了<code>IntSet</code> ，然后Override了 它的 <code>Add()</code>和 <code>Delete()</code> 方法；</li><li><code>Contains()</code> 方法没有Override，所以，就被带到 <code>UndoableInSet</code> 中来了。</li><li>在Override的 <code>Add()</code>中，记录 <code>Delete</code> 操作；</li><li>在Override的 <code>Delete()</code> 中，记录 <code>Add</code> 操作；</li><li>在新加入的 <code>Undo()</code> 中进行Undo操作。</li></ul><p>用这样的方式为已有的代码扩展新的功能是一个很好的选择。这样，就可以在重用原有代码功能和新的功能中达到一个平衡。但是，这种方式最大的问题是，Undo操作其实是一种控制逻辑，并不是业务逻辑，所以，在复用 Undo这个功能时，是有问题的，因为其中加入了大量跟 <code>IntSet</code> 相关的业务逻辑。</p><h3 id="反转依赖" tabindex="-1">反转依赖 <a class="header-anchor" href="#反转依赖" aria-label="Permalink to &quot;反转依赖&quot;">&amp;ZeroWidthSpace;</a></h3><p>现在我们来看另一种方法。</p><p>我们先声明一种函数接口，表示我们的Undo控制可以接受的函数签名是什么样的：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>type Undo []func()</span></span></code></pre></div><p>有了这个协议之后，我们的Undo控制逻辑就可以写成下面这样：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>func (undo *Undo) Add(function func()) {</span></span>
...
<span class="line"><span>        set.undo.Add(nil)</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>这个就是控制反转，不是由控制逻辑 <code>Undo</code> 来依赖业务逻辑 <code>IntSet</code>，而是由业务逻辑 <code>IntSet</code> 依赖 <code>Undo</code> 。这里依赖的是其实是一个协议，**这个协议是一个没有参数的函数数组。**可以看到，这样一来，我们 Undo 的代码就可以复用了。</p><p>好了，这节课就到这里。如果你觉得今天的内容对你有所帮助，欢迎你帮我分享给更多人。 精选留言（8） limix 👍（5） 💬（0）控制逻辑的特征是可复用性比较高，多场景可用，而业务逻辑的特征是专用性，控制逻辑，应该复用控制逻辑，而不是复用也许逻辑2022-09-07debugtalk 👍（4） 💬（0）手动点赞2021-06-12cvvz 👍（1） 💬（0）反转控制： 第一种改写方法——继承，好处是原有代码可以复用，控制逻辑和业务逻辑解耦，坏处是通用的控制逻辑无法复用 第二种改写方法——反转控制，好处是复用通用的控制逻辑，坏处是要修改原有代码的逻辑，把控制逻辑嵌入到了业务逻辑中2022-08-27Geek_ce6971 👍（0） 💬（1）实现undo功能，有个地方是写反了吗 在 Override 的 Add()中，记录 Delete 操作；在 Override 的 Delete() 中，记录 Add 操作；2022-01-19方勇(gopher) 👍（0） 💬（0）日常中有些装饰器其实可以用这种方式替换2021-12-20衡子 👍（0） 💬（0）厉害了👍2021-11-30一光年 👍（0） 💬（0）控制逻辑依赖业务逻辑，不如让业务容器依赖控制逻辑2021-08-11Geek_46da16 👍（0） 💬（0）会玩，2021-07-26</p>`,46)])])}const h=s(t,[["render",l]]);export{b as __pageData,h as default};
