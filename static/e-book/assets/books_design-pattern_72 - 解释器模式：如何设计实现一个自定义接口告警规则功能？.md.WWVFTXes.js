...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><h2 id="重点回顾" tabindex="-1">重点回顾 <a class="header-anchor" href="#重点回顾" aria-label="Permalink to &quot;重点回顾&quot;">&amp;ZeroWidthSpace;</a></h2><p>好了，今天的内容到此就讲完了。我们一块来总结回顾一下，你需要重点掌握的内容。</p><p>解释器模式为某个语言定义它的语法（或者叫文法）表示，并定义一个解释器用来处理这个语法。实际上，这里的“语言”不仅仅指我们平时说的中、英、日、法等各种语言。从广义上来讲，只要是能承载信息的载体，我们都可以称之为“语言”，比如，古代的结绳记事、盲文、哑语、摩斯密码等。</p><p>要想了解“语言”要表达的信息，我们就必须定义相应的语法规则。这样，书写者就可以根据语法规则来书写“句子”（专业点的叫法应该是“表达式”），阅读者根据语法规则来阅读“句子”，这样才能做到信息的正确传递。而我们要讲的解释器模式，其实就是用来实现根据语法规则解读“句子”的解释器。</p><p>解释器模式的代码实现比较灵活，没有固定的模板。我们前面说过，应用设计模式主要是应对代码的复杂性，解释器模式也不例外。它的代码实现的核心思想，就是将语法解析的工作拆分到各个小类中，以此来避免大而全的解析类。一般的做法是，将语法规则拆分一些小的独立的单元，然后对每个单元进行解析，最终合并为对整个语法规则的解析。</p><h2 id="课堂讨论" tabindex="-1">课堂讨论 <a class="header-anchor" href="#课堂讨论" aria-label="Permalink to &quot;课堂讨论&quot;">&amp;ZeroWidthSpace;</a></h2><p>1.在你过往的项目经历或阅读源码的时候，有没有用到或者见过解释器模式呢？<br> 2.在告警规则解析的例子中，如果我们要在表达式中支持括号“（）”，那如何对代码进行重构呢？你可以把它当作练习，试着编写一下代码。</p><p>欢迎留言和我分享你的想法。如果有收获，也欢迎你把这篇文章分享给你的朋友。 精选留言（15） 花儿少年 👍（11） 💬（1）类似于规则引擎是不是也是个解释器，只不过是个复杂很多的解释器2020-04-27Panmax 👍（68） 💬（8）第二个代码示例中的代码段： if (operator.equals(&quot;+&quot;)) { combinedExp = new AdditionExpression(exp1, exp2); } else if (operator.equals(&quot;-&quot;)) { combinedExp = new AdditionExpression(exp1, exp2); } else if (operator.equals(&quot;<em>&quot;)) { combinedExp = new AdditionExpression(exp1, exp2); } else if (operator.equals(&quot;/&quot;)) { combinedExp = new AdditionExpression(exp1, exp2); } 应改为： if (operator.equals(&quot;+&quot;)) { combinedExp = new AdditionExpression(exp1, exp2); } else if (operator.equals(&quot;-&quot;)) { combinedExp = new SubstractionExpression(exp1, exp2); } else if (operator.equals(&quot;</em>&quot;)) { combinedExp = new MultiplicationExpression(exp1, exp2); } else if (operator.equals(&quot;/&quot;)) { combinedExp = new DivisionExpression(exp1, exp2); }2020-04-18LiuHu 👍（42） 💬（0）SpEL（Spring Expression Language ) 就是典型的解释器模式实现。 在项目中实现简单的自动化规则过滤器，就是将过滤器配置翻译成 SpEL 表达式执行的2020-05-05javaadu 👍（21） 💬（0）我是做风控技术的，发现这个模式对我理解规则引擎的设计很有帮助2020-04-29迷羊 👍（16） 💬（3）因为做的是数据类型的项目，就是根据自定义的SQL来创建API以供可以通过http形式直接调用，项目中就是用Druid来解析SQL的，用的就是解释器模式，SQL语句中的每个字符对应一个表达式。2020-04-18kylexy_0817 👍（11） 💬（0）ES中的各种分词器2020-04-25Ken张云忠 👍（11） 💬（2）Java中注解处理器做的就是解释的功能，以及前端编译时的语法分析、语义分析，后端编译时生成的中间表达式，用来触发更多优化，优化的处理可以理解为高效的解释，最终生成机器可以执行的汇编指令。2020-04-17鲨鱼辣椒 👍（10） 💬（0）定时任务中的cron表达式2020-05-20辣么大 👍（10） 💬（0）关于问题一，使用过。偏向科研，自定义一门语言，然后通过语法解析器分析读入。 例如使用RDDL(Relational Domain Definition Language)关系领域定义语言描述马尔可夫决策过程。 domain prop_dbn {</p><pre><code>requirements = { reward-deterministic };
...
</code></pre><p>}</p><p>instance inst_dbn {</p><pre><code>domain = prop_dbn;	
init-state { 
...
horizon  = 20;
discount = 0.9;
</code></pre><p>}2020-04-17test 👍（7） 💬（1）加括号的话，要加一个ExpressionManager，在manager里面用括号把表达式划分为几段，再根据表达式间是 与 还是 或 来添加最上面那一层的表达式2020-04-17李稳 👍（6） 💬（0）输入规则，解析规则，输入数据，使用规则判断数据2020-04-17苗 👍（5） 💬（0）使用过nutz开源项目的表达式引擎，可以扩展各种操作符。2020-04-29马球先生 👍（5） 💬（0）告警规则的代码 真是读了半天才看懂😂2020-04-18Heaven 👍（5） 💬（3）https://github.com/HeavenXin/MonitorExpressionTest 关于第二道题,趁着中午写了写,大家可以参考一下2020-04-17南山 👍（3） 💬（0）还真有项目有这种场景，动态表单的值校验，可以自定义校验，校验之间还可以相互组合。但是用的是组合模式，后面思考一下解释器模式如何使用，貌似更适合~2020-04-21</p>`,49)])])}const m=n(l,[["render",i]]);export{x as __pageData,m as default};
