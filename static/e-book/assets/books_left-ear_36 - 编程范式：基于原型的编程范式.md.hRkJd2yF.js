import{_ as s,o as n,c as e,ae as p}from"./chunks/framework.Iv6F95cJ.js";const h=JSON.parse('{"title":"JavaScript的原型概念","description":"","frontmatter":{},"headers":[],"relativePath":"books/left-ear/36 - 编程范式：基于原型的编程范式.md","filePath":"books/left-ear/36 - 编程范式：基于原型的编程范式.md"}'),o={name:"books/left-ear/36 - 编程范式：基于原型的编程范式.md"};function t(l,a,c,i,r,d){return n(),e("div",null,[...a[0]||(a[0]=[p(`<p>你好，我是陈皓，网名左耳朵耗子。</p><p>基于原型（Prototype）的编程其实也是面向对象编程的一种方式。没有class化的，直接使用对象。又叫，基于实例的编程。其主流的语言就是JavaScript，与传统的面向对象编程的比较如下：</p><ul><li>在基于类的编程当中，对象总共有两种类型。类定义了对象的基本布局和函数特性，而接口是“可以使用的”对象，它基于特定类的样式。在此模型中，类表现为行为和结构的集合，对所有接口来说这些类的行为和结构都是相同的。因而，区分规则首先是基于行为和结构，而后才是状态。</li><li>原型编程的主张者经常争论说，基于类的语言提倡使用一个关注分类和类之间关系的开发模型。与此相对，原型编程看起来提倡程序员关注一系列对象实例的行为，而之后才关心如何将这些对象划分到最近的使用方式相似的原型对象，而不是分成类。</li></ul><p>因为如此，很多基于原型的系统提倡运行时进行原型的修改，而只有极少数基于类的面向对象系统（比如第一个动态面向对象的系统Smalltalk）允许类在程序运行时被修改。</p><ul><li>在基于类的语言中，一个新的实例通过类构造器和构造器可选的参数来构造，结果实例由类选定的行为和布局创建模型。</li><li>在基于原型的系统中构造对象有两种方法，通过复制已有的对象或者通过扩展空对象创建。很多基于原型的系统提倡运行时进行原型的修改，而基于类的面向对象系统只有动态语言允许类在运行时被修改（Common Lisp、Dylan、Objective-C、Perl、Python、Ruby和Smalltalk）。</li></ul><h1 id="javascript的原型概念" tabindex="-1">JavaScript的原型概念 <a class="header-anchor" href="#javascript的原型概念" aria-label="Permalink to &quot;JavaScript的原型概念&quot;">&amp;ZeroWidthSpace;</a></h1><p>这里，我们主要以JavaScript举例，面向对象里面要有个Class。但是JavaScript觉得不是这样的，它就是要基于原型编程，就不要Class，就直接在对象上改就行了，基于编程的修改，直接对类型进行修改。</p><p>我们先来看一个示例。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>var foo = {name: &quot;foo&quot;, one: 1, two: 2};</span></span>
...
<span class="line"><span>c.calculate(40); // 80</span></span></code></pre></div><p>其中的“原型链”如下所示：</p><p><img src="https://static001.geekbang.org/resource/image/a6/ec/a67686c883e8077da6779ae956efdcec.png?wh=378x244" alt="" loading="lazy" referrerpolicy="no-referrer"></p><p>注意：ES5 中，规定原型继承需要使用 <code>Object.create()</code> 函数。如下所示：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>var b = Object.create(a, {y: {value: 20}});</span></span>
...
<span class="line"><span>b.calculate(30); // 60</span></span>
<span class="line"><span>c.calculate(40); // 80</span></span></code></pre></div><p>那么，在内存中的布局是怎么样的呢？大概是下面这个样子。</p><p><img src="https://static001.geekbang.org/resource/image/6c/fa/6ca465981ae68f4a055f9ca4f16fdffa.png?wh=635x396" alt="" loading="lazy" referrerpolicy="no-referrer"></p><p>这个图应该可以让你很好地看明白 <code>__proto__</code> 和 <code>prototype</code> 的差别了。</p><p>我们可以测试一下：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>b.__proto__ === Foo.prototype, // true</span></span>
...
<span class="line"><span>  };</span></span>
<span class="line"><span>};</span></span></code></pre></div><p>在上面的代码中：</p><ul><li>使用了 <code>Person.call(this, fullName, email)</code>，<code>call()</code> 或 <code>apply()</code> 都是为了动态改变 <code>this</code> 所指向的对象的内容而出现的。这里的 <code>this</code> 就是 <code>Student</code>。</li><li>上面的例子中，我们重载了 <code>introduction()</code> 方法，并新增加了一个 <code>takeExams()</code>的方法。</li></ul><p>虽然，我们这样定义了 <code>Student</code>，但是它还没有和 <code>Person</code> 发生继承关系。为了要让它们发生关系，我们就需要修改 <code>Student</code> 的原型。</p><p>我们可以简单粗暴地做赋值：<code>Student.__proto__ = Person.prototype</code> ，但是，这太粗暴了。</p><p>我们还是使用比较规范的方式：</p><ul><li>先用 <code>Object.create()</code> 来将<code>Person.prototype</code> 和 <code>Student.prototype</code> 关联上。</li><li>然后，修改一下构造函数 <code>Student.prototype.constructor = Student;</code>。</li></ul><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>// Create a Student.prototype object that inherits </span></span>
...
<span class="line"><span>console.log(student instanceof Student); // true</span></span></code></pre></div><p>上述就是基于原型的面向对象编程的玩法了。</p><blockquote><p>注：在ECMAScript标准的第四版开始寻求使JavaScript提供基于类的构造，且ECMAScript第六版有提供&quot;class&quot;(类)作为原有的原型架构之上的语法糖，提供构建对象与处理继承时的另一种语法。</p></blockquote><h1 id="小结" tabindex="-1">小结 <a class="header-anchor" href="#小结" aria-label="Permalink to &quot;小结&quot;">&amp;ZeroWidthSpace;</a></h1><p>我们可以看到，这种玩法就是一种委托的方式。在使用委托的基于原型的语言中，运行时语言可以“仅仅通过序列的指针找到匹配”这样的方式来定位属性或者寻找正确的数据。所有这些创建行为、共享的行为需要的是委托指针。</p><p>不像是基于类的面向对象语言中类和接口的关系，原型和它的分支之间的关系并不要求子对象有相似的内存结构，因为如此，子对象可以继续修改而无需像基于类的系统那样整理结构。还有一个要提到的地方是，不仅仅是数据，方法也能被修改。因为这个原因，大多数基于原型的语言把数据和方法提作“slots”。</p><p>这种在对象里面直接修改的玩法，虽然这个特性可以带来运行时的灵活性，我们可以在运行时修改一个prototype，给它增加甚至删除属性和方法。但是其带来了执行的不确定性，也有安全性的问题，而代码还变得不可预测，这有点黑科技的味道了。因为这些不像静态类型系统，没有一个不可变的契约对代码的确定性有保证，所以，需要使用者来自己保证。</p><p>以下是《编程范式游记》系列文章的目录，方便你了解这一系列内容的全貌。</p><ul><li><a href="https://time.geekbang.org/column/article/301" target="_blank" rel="noreferrer">01 | 编程范式游记：起源</a></li><li><a href="https://time.geekbang.org/column/article/303" target="_blank" rel="noreferrer">02 | 编程范式游记：泛型编程</a></li><li><a href="https://time.geekbang.org/column/article/2017" target="_blank" rel="noreferrer">03 | 编程范式游记：类型系统和泛型的本质</a></li><li><a href="https://time.geekbang.org/column/article/2711" target="_blank" rel="noreferrer">04 | 编程范式游记：函数式编程</a></li><li><a href="https://time.geekbang.org/column/article/2723" target="_blank" rel="noreferrer">05 | 编程范式游记：修饰器模式</a></li><li><a href="https://time.geekbang.org/column/article/2729" target="_blank" rel="noreferrer">06 | 编程范式游记：面向对象编程</a></li><li><a href="https://time.geekbang.org/column/article/2741" target="_blank" rel="noreferrer">07 | 编程范式游记：基于原型的编程范式</a></li><li><a href="https://time.geekbang.org/column/article/2748" target="_blank" rel="noreferrer">08 | 编程范式游记：Go 语言的委托模式</a></li><li><a href="https://time.geekbang.org/column/article/2751" target="_blank" rel="noreferrer">09 | 编程范式游记：编程的本质</a></li><li><a href="https://time.geekbang.org/column/article/2752" target="_blank" rel="noreferrer">10 | 编程范式游记：逻辑编程范式</a></li><li><a href="https://time.geekbang.org/column/article/2754" target="_blank" rel="noreferrer">11 | 编程范式游记：程序世界里的编程范式</a> 精选留言（15） 云学 👍（12） 💬（0）c++的原型模式就是一个clone虚方法，这篇文章看得是很懵逼，完全不了解Javascript2018-02-02yun 👍（6） 💬（3）皓叔，是否可以分享下如何写出高质量的代码的经验？排查问题的方法经验？2018-02-19高科 👍（5） 💬（1）曾经以为自己懂JS，结果发现还真是以为2020-04-27edisonhuang 👍（4） 💬（0）基于原型的编程可以理解为是基于实例的编程，这种玩法就是一种委托的方式。在使用委托的基于原型的语言中，运行时语言可以“仅仅通过序列的指针找到匹配”这样的方式来定位属性或者寻找正确的数据。所有这些创建行为共享的行为需要的是委托指针。js使用基于原型的编程方法2019-06-25fsj 👍（4） 💬（3）看了这篇文章，然后又去翻了《JavaScript高级程序设计》，终于把原型继承搞明白了。之前学习Objective-C，觉得这语法挺奇葩的，但是面向对象思想和Java等都是互通的，然后又学习了JS，算是更奇葩了，没有类，玩的是原型继承，也是服，为什么JS不搞成类继承，非要搞一个原型继承呢？2018-04-08强 👍（4） 💬（0）老师，有时间能写写go吗2018-02-02蜉蝣 👍（2） 💬（0）看过之前完全不懂，看完之后懵懵懂懂。不晓得什么时候才能搞懂。2019-08-19杨洪林 👍（2） 💬（0）引用于JavaScript, The Core的图例有些问题，Function.prototype的__proto__应该是指向Object.prototype, 似乎Object.prototype应该是所有非prime类型对象的根2019-02-21bqzzz 👍（1） 💬（0）最后一个图是错了还是JavaScript更新了机制？ 现在原文链接中Function.prototype.<strong>proto</strong> === Object.prototype，而专栏中的Function.prototype.__proto__指向自身？2022-03-18刘匿名 👍（1） 💬（0）js 的面向对象方式选择了灵活性，一定程度上失去了安全性和可预测性。 以前用过js，灵活简单易上手，但是同时项目复杂时就会很难排查问题，后来兴起了一系列前端项目的构建工具，和js的这些特点也不无关系。 每门语言都有自己的设计初衷和适用场景，现在常见的开发语言，设计思路上都有很多精华值得学习。 皓哥这篇文章把js面向对象的精华深入浅出的总结了下，赞！2020-12-10wWw 👍（1） 💬（0）类定义了对象的基本布局和函数特性，而接口是“可以使用的”对象，它基于特定类的样式。在此模型中，类表现为行为和结构的集合，对所有接口来说这些类的行为和结构都是相同的。因而，区分规则首先是基于行为和结构，而后才是状态。</li></ul><p>这里的“接口是可以使用的对象”如何理解？对所有接口来说类的行为和结构是相同的，这个也没理解？麻烦理解的学友帮忙解释一下2020-10-17ck 👍（1） 💬（0）多读了几遍后. 猜想&quot;可以使用&quot; 是说接口只定义了抽象方法. 而用户可以根据接口直接使用它类型的对象而不用担心属性值么?2020-07-26ck 👍（1） 💬（1）老师: Java中接口是类的一种抽象. 为什么说接口是&quot;可以使用的&quot;对象. 如果 &quot;使用&quot; 接口作为一种类型的话, 那类也是一种类型. 他们都是可使用的.2020-07-26slark 👍（1） 💬（0）基于原型的面向对象的编程语言。js日常也有使用到，不过，只知道是动态运行时语言，对其原型概念并不懂。或者日常也不使用这个特性。从文章介绍，可以看到存在很多灵活性。存在这种逻辑也是很有趣的2020-01-12磉盘 👍（1） 💬（0）理解了原型模式，才能更好地使用面向对象思想写JavaScript代码。2019-09-24</p>`,67)])])}const g=s(o,[["render",t]]);export{h as __pageData,g as default};
