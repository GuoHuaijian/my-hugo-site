import{_ as s,o as n,c as e,ae as p}from"./chunks/framework.Iv6F95cJ.js";const g=JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"books/java-practice/33 - Thread-Per-Message模式：最简单实用的分工方法.md","filePath":"books/java-practice/33 - Thread-Per-Message模式：最简单实用的分工方法.md"}'),l={name:"books/java-practice/33 - Thread-Per-Message模式：最简单实用的分工方法.md"};function r(i,a,c,t,o,d){return n(),e("div",null,[...a[0]||(a[0]=[p(`<p>我们曾经把并发编程领域的问题总结为三个核心问题：分工、同步和互斥。其中，同步和互斥相关问题更多地源自微观，而分工问题则是源自宏观。我们解决问题，往往都是从宏观入手，在编程领域，软件的设计过程也是先从概要设计开始，而后才进行详细设计。同样，<strong>解决并发编程问题，首要问题也是解决宏观的分工问题</strong>。</p><p>并发编程领域里，解决分工问题也有一系列的设计模式，比较常用的主要有Thread-Per-Message模式、Worker Thread模式、生产者-消费者模式等等。今天我们重点介绍Thread-Per-Message模式。</p><h2 id="如何理解thread-per-message模式" tabindex="-1">如何理解Thread-Per-Message模式 <a class="header-anchor" href="#如何理解thread-per-message模式" aria-label="Permalink to &quot;如何理解Thread-Per-Message模式&quot;">&amp;ZeroWidthSpace;</a></h2><p>现实世界里，很多事情我们都需要委托他人办理，一方面受限于我们的能力，总有很多搞不定的事，比如教育小朋友，搞不定怎么办呢？只能委托学校老师了；另一方面受限于我们的时间，比如忙着写Bug，哪有时间买别墅呢？只能委托房产中介了。委托他人代办有一个非常大的好处，那就是可以专心做自己的事了。</p><p>在编程领域也有很多类似的需求，比如写一个HTTP Server，很显然只能在主线程中接收请求，而不能处理HTTP请求，因为如果在主线程中处理HTTP请求的话，那同一时间只能处理一个请求，太慢了！怎么办呢？可以利用代办的思路，创建一个子线程，委托子线程去处理HTTP请求。</p><p>这种委托他人办理的方式，在并发编程领域被总结为一种设计模式，叫做<strong>Thread-Per-Message模式</strong>，简言之就是为每个任务分配一个独立的线程。这是一种最简单的分工方法，实现起来也非常简单。</p><h2 id="用thread实现thread-per-message模式" tabindex="-1">用Thread实现Thread-Per-Message模式 <a class="header-anchor" href="#用thread实现thread-per-message模式" aria-label="Permalink to &quot;用Thread实现Thread-Per-Message模式&quot;">&amp;ZeroWidthSpace;</a></h2><p>Thread-Per-Message模式的一个最经典的应用场景是<strong>网络编程里服务端的实现</strong>，服务端为每个客户端请求创建一个独立的线程，当线程处理完请求后，自动销毁，这是一种最简单的并发处理网络请求的方法。</p><p>网络编程里最简单的程序当数echo程序了，echo程序的服务端会原封不动地将客户端的请求发送回客户端。例如，客户端发送TCP请求&quot;Hello World&quot;，那么服务端也会返回&quot;Hello World&quot;。</p><p>下面我们就以echo程序的服务端为例，介绍如何实现Thread-Per-Message模式。</p><p>在Java语言中，实现echo程序的服务端还是很简单的。只需要30行代码就能够实现，示例代码如下，我们为每个请求都创建了一个Java线程，核心代码是：new Thread(()-&gt;{...}).start()。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>final ServerSocketChannel  = </span></span>
...
<span class="line"><span>Total:       2000 2567 3543.9   2004   65293</span></span></code></pre></div><p>你会发现即便在20000并发下，该程序依然能够良好运行。同等条件下，Thread实现的echo程序512并发都抗不过去，直接就OOM了。</p><p>如果你通过Linux命令 <code>top -Hp pid</code> 查看Fiber实现的echo程序的进程信息，你可以看到该进程仅仅创建了16（不同CPU核数结果会不同）个操作系统线程。</p><p><img src="https://static001.geekbang.org/resource/image/ae/e9/aebe9691be206fb88f45e4f763bcb7e9.png?wh=826%2A512" alt="" loading="lazy" referrerpolicy="no-referrer"></p><p>如果你对Loom项目感兴趣，也想上手试一把，可以下载源代码自己构建，构建方法可以参考<a href="https://wiki.openjdk.java.net/display/loom/Main" target="_blank" rel="noreferrer">Project Loom的相关资料</a>，不过需要注意的是构建之前一定要把代码分支切换到Fibers。</p><h2 id="总结" tabindex="-1">总结 <a class="header-anchor" href="#总结" aria-label="Permalink to &quot;总结&quot;">&amp;ZeroWidthSpace;</a></h2><p>并发编程领域的分工问题，指的是如何高效地拆解任务并分配给线程。前面我们在并发工具类模块中已经介绍了不少解决分工问题的工具类，例如Future、CompletableFuture 、CompletionService、Fork/Join计算框架等，这些工具类都能很好地解决特定应用场景的问题，所以，这些工具类曾经是Java语言引以为傲的。不过这些工具类都继承了Java语言的老毛病：太复杂。</p><p>如果你一直从事Java开发，估计你已经习以为常了，习惯性地认为这个复杂度是正常的。不过这个世界时刻都在变化，曾经正常的复杂度，现在看来也许就已经没有必要了，例如Thread-Per-Message模式如果使用线程池方案就会增加复杂度。</p><p>Thread-Per-Message模式在Java领域并不是那么知名，根本原因在于Java语言里的线程是一个重量级的对象，为每一个任务创建一个线程成本太高，尤其是在高并发领域，基本就不具备可行性。不过这个背景条件目前正在发生巨变，Java语言未来一定会提供轻量级线程，这样基于轻量级线程实现Thread-Per-Message模式就是一个非常靠谱的选择。</p><p>当然，对于一些并发度没那么高的异步场景，例如定时任务，采用Thread-Per-Message模式是完全没有问题的。实际工作中，我就见过完全基于Thread-Per-Message模式实现的分布式调度框架，这个框架为每个定时任务都分配了一个独立的线程。</p><h2 id="课后思考" tabindex="-1">课后思考 <a class="header-anchor" href="#课后思考" aria-label="Permalink to &quot;课后思考&quot;">&amp;ZeroWidthSpace;</a></h2><p>使用Thread-Per-Message模式会为每一个任务都创建一个线程，在高并发场景中，很容易导致应用OOM，那有什么办法可以快速解决呢？</p><p>欢迎在留言区与我分享你的想法，也欢迎你在留言区记录你的思考过程。感谢阅读，如果你觉得这篇文章对你有帮助的话，也欢迎把它分享给更多的朋友。 精选留言（15） 唯美 👍（95） 💬（3）快速解决，就只能改jvm内存配置，增大jvm新生代的大小，长期解决，引入NIO或AIO，netty 就是这么干的2019-05-14QQ怪 👍（38） 💬（3）之前一直听别人说go语言很容易抗并发，原来是有个这么牛逼的轻量级线程在啊，并且理解了基本原理，感谢老师分享，为自己扩充了眼界了，为老师打call👍2019-05-14韦 👍（32） 💬（1）每次创建一个线程高并发肯定OOM，1引入线程池控制创建线程的大小，通过压测得到比较合理的线程数量配置，2 需要在请求端增加一个限流模块，自我保护2019-05-24245864982 👍（16） 💬（1）老师，那tomcat也是一个请求一个线程的吧？它是怎么解决的。是用线程池吗？2019-11-28欢乐小熊 👍（13） 💬（2）Kotlin 支持协程, 并且实现了完善的调度策略, 可以考虑使用 Kotlin 作为替代方案2019-09-30cricket1981 👍（13） 💬（3）网上查资料说java中的线程是内核空间的，轻量级线程属于用户空间的，是这样吗？另外轻量级线程是如何调度的？2019-05-17考休 👍（4） 💬（1）go中的轻量级线程就是协程吧？2019-11-06Sam 👍（3） 💬（1）老师，请问Java不支持轻量级线程，但是有Fibe，那是不是就可以用Fibe在实际项目中，然后相当于Java也支持轻量级线程了？2019-08-01旅途 👍（2） 💬（1）老师 问一下 fiber并发这么牛 为什么tomcat不使用fiber呢2020-03-01尔东橙 👍（1） 💬（1）老师，轻量级线程语言层次实现的，该怎么理解2019-07-21苗 👍（0） 💬（1）用个队列限流是不是通用思路？2020-06-20ly 👍（0） 💬（1）老师，能不能提供一个编译openjdk的方法，我在网上搜索了好多博客，按照他们说的步骤，都出现各种各样的问题，难道编译openjdk这么难？主要是长期用window系统，搞起好麻烦！2019-10-09夕夕熊 👍（0） 💬（1）tomcat 、webLogic 用的是这种模式吗2019-05-14colin 👍（0） 💬（1）我想试试Fiber，这个是不是要使用OpenJDK2019-05-14~Wade 👍（25） 💬（1）有一个java库叫Quasar Fiber ，通过javaagent技术可以实现轻量级线程<br> 官网：http://www.paralleluniverse.co/quasar/2019-06-21</p>`,36)])])}const b=s(l,[["render",r]]);export{g as __pageData,b as default};
