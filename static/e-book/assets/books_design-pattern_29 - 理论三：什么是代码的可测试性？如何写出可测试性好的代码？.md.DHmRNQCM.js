...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>上面的单元测试有可能会运行失败。假设单元测试框架顺序依次执行testMove_betweenRange()和testMove_exceedRange()两个测试用例。在第一个测试用例执行完成之后，position的值变成了-1；再执行第二个测试用例的时候，position变成了5，move()函数返回true，assertFalse语句判定失败。所以，第二个测试用例运行失败。</p><p>当然，如果RangeLimiter类有暴露重设（reset）position值的函数，我们可以在每次执行单元测试用例之前，把position重设为0，这样就能解决刚刚的问题。</p><p>不过，每个单元测试框架执行单元测试用例的方式可能是不同的。有的是顺序执行，有的是并发执行。对于并发执行的情况，即便我们每次都把position重设为0，也并不奏效。如果两个测试用例并发执行，第16、17、18、23这四行代码可能会交叉执行，影响到move()函数的执行结果。</p><h3 id="_3-静态方法" tabindex="-1">3.静态方法 <a class="header-anchor" href="#_3-静态方法" aria-label="Permalink to &quot;3.静态方法&quot;">&amp;ZeroWidthSpace;</a></h3><p>前面我们也提到，静态方法跟全局变量一样，也是一种面向过程的编程思维。在代码中调用静态方法，有时候会导致代码不易测试。主要原因是静态方法也很难mock。但是，这个要分情况来看。只有在这个静态方法执行耗时太长、依赖外部资源、逻辑复杂、行为未决等情况下，我们才需要在单元测试中mock这个静态方法。除此之外，如果只是类似Math.abs()这样的简单静态方法，并不会影响代码的可测试性，因为本身并不需要mock。</p><h3 id="_4-复杂继承" tabindex="-1">4.复杂继承 <a class="header-anchor" href="#_4-复杂继承" aria-label="Permalink to &quot;4.复杂继承&quot;">&amp;ZeroWidthSpace;</a></h3><p>我们前面提到，相比组合关系，继承关系的代码结构更加耦合、不灵活，更加不易扩展、不易维护。实际上，继承关系也更加难测试。这也印证了代码的可测试性跟代码质量的相关性。</p><p>如果父类需要mock某个依赖对象才能进行单元测试，那所有的子类、子类的子类……在编写单元测试的时候，都要mock这个依赖对象。对于层次很深（在继承关系类图中表现为纵向深度）、结构复杂（在继承关系类图中表现为横向广度）的继承关系，越底层的子类要mock的对象可能就会越多，这样就会导致，底层子类在写单元测试的时候，要一个一个mock很多依赖对象，而且还需要查看父类代码，去了解该如何mock这些依赖对象。</p><p>如果我们利用组合而非继承来组织类之间的关系，类之间的结构层次比较扁平，在编写单元测试的时候，只需要mock类所组合依赖的对象即可。</p><h3 id="_5-高耦合代码" tabindex="-1">5.高耦合代码 <a class="header-anchor" href="#_5-高耦合代码" aria-label="Permalink to &quot;5.高耦合代码&quot;">&amp;ZeroWidthSpace;</a></h3><p>如果一个类职责很重，需要依赖十几个外部对象才能完成工作，代码高度耦合，那我们在编写单元测试的时候，可能需要mock这十几个依赖的对象。不管是从代码设计的角度来说，还是从编写单元测试的角度来说，这都是不合理的。</p><h2 id="重点回顾" tabindex="-1">重点回顾 <a class="header-anchor" href="#重点回顾" aria-label="Permalink to &quot;重点回顾&quot;">&amp;ZeroWidthSpace;</a></h2><p>好了，今天的内容到此就讲完了。我们一块来总结回顾一下，你需要重点掌握的内容。</p><p><strong>1.什么是代码的可测试性？</strong></p><p>粗略地讲，所谓代码的可测试性，就是针对代码编写单元测试的难易程度。对于一段代码，如果很难为其编写单元测试，或者单元测试写起来很费劲，需要依靠单元测试框架中很高级的特性，那往往就意味着代码设计得不够合理，代码的可测试性不好。</p><p><strong>2.编写可测试性代码的最有效手段</strong></p><p>依赖注入是编写可测试性代码的最有效手段。通过依赖注入，我们在编写单元测试的时候，可以通过mock的方法解依赖外部服务，这也是我们在编写单元测试的过程中最有技术挑战的地方。</p><p><strong>3.常见的Anti-Patterns</strong></p><p>常见的测试不友好的代码有下面这5种：</p><ul><li>代码中包含未决行为逻辑</li><li>滥用可变全局变量</li><li>滥用静态方法</li><li>使用复杂的继承关系</li><li>高度耦合的代码</li></ul><h2 id="课堂讨论" tabindex="-1">课堂讨论 <a class="header-anchor" href="#课堂讨论" aria-label="Permalink to &quot;课堂讨论&quot;">&amp;ZeroWidthSpace;</a></h2><ol><li>实战案例中的void fillTransactionId(String preAssignedId)函数中包含一处静态函数调用：IdGenerator.generateTransactionId()，这是否会影响到代码的可测试性？在写单元测试的时候，我们是否需要mock这个函数？</li><li>我们今天讲到，依赖注入是提高代码可测试性的最有效的手段。所以，依赖注入，就是不要在类内部通过new的方式创建对象，而是要通过外部创建好之后传递给类使用。那是不是所有的对象都不能在类内部创建呢？哪种类型的对象可以在类内部创建并且不影响代码的可测试性？你能举几个例子吗？</li></ol><p>欢迎在留言区写下你的答案，和同学一起交流和分享。如果有收获，也欢迎你把这篇文章分享给你的朋友。 精选留言（15） 楊_宵夜 👍（78） 💬（10）争歌, 代码中isExpired()方法的修饰符是protected, 如果某些方法从设计原则来说应该设置为private的话, 那么这样的手动mock的方式是否就不适用了呢? 换个角度来提问: 为了维持可测试性, 在代码中加入过多protected的方法, 是否合理呢?2020-01-20张三丰 👍（23） 💬（1）老师，下面这句话不是很理解，如果我的某个接口就是需要依赖很多服务才能把结果正确返回给前端，这时候怎么办？比如查询购物车，需要访问商品服务的商品信息，优惠服务的优惠信息，同时访问价格服务的价格信息等等，这个时候，高度耦合怎么去避免呢？</p><p>如果一个类职责很重，需要依赖十几个外部对象才能完成工作，代码高度耦合，那我们在编写单元测试的时候，可能需要 mock 这十几个依赖的对象。不管是从代码设计的角度来说，还是从编写单元测试的角度来说，这都是不合理的。2020-05-28QQ怪 👍（18） 💬（2）看到一半，我就来评论，老师收下我的膝盖，太强了2020-01-08Vincent.X 👍（6） 💬（1）手机看代码有老是要拖动，有什么解决的办法吗？？2020-06-17美美 👍（5） 💬（4）有多个通过spring注入的类时，应该怎么做测试呢？2020-01-08Mew151 👍（1） 💬（1）有一个问题，如果测试方法A()中调用了本类的私有方法B()，这个时候该怎么处理呢？2020-08-26J.Smile 👍（1） 💬（2）想到一个问题，代码结构扁平化的极端结果可能会造成依赖对象过多吗？这种情况mock不是依然难搞吗2020-01-08qpzm7903 👍（0） 💬（1）请问贫血模式的mvc中的service怎么进行单元测试呢2020-04-26失火的夏天 👍（179） 💬（2）思考题1，该方法逻辑就是填充一个ID，基本都是内部实现的一个id生成器，可以不用重写。一定要重写也行，自己弄一个自增id实现就行了。 思考题2，提供方法的类不要new，也就是我们常说的service类，这个是要依赖注入的。提供属性的类，比如vo，bo，entity这些就可以new。2020-01-08安静的boy 👍（62） 💬（0）这节满满的干货👍👍👍2020-01-08辣么大 👍（39） 💬（7）参考争哥今天的代码写了例子中的测试（可运行）： https://github.com/gdhucoder/Algorithms4/tree/master/designpattern/u29</p><p>今天学习到了高级的单元测试方法： 1、依赖外部单例：将单例封装 2、未决行为：例时间、随机数。将未决行为重新封装，测试时mock，使用匿名类。</p><p>关于讨论1：需要mock的情况id会写入数据库的话，测试后需要恢复现场。曾经遇到过这么一个情况，id是通过一张表维护的，大于0，在代码中id的数据类型是Integer（遗留代码），由于测试时没有恢复现场，导致测试数据库中id增加过快，超过了代码中Integer的表示范围，而产生了意想不到的问题。2020-01-08桂城老托尼 👍（20） 💬（2）感谢争哥分享 课后讨论1.id的生成逻辑有点没看懂，单纯从代码覆盖上看，fillTransactionId 未覆盖完全，需要mock下这个静态方法，当然也有其他分支逻辑可以覆盖。 id没有在execute方法中不是核心属性(mock方法的入参)，不影响execute的可测试性。 id的生成用静态方法真的好么？ 2.有行为的对象不适合在类中new，尽量使用依赖注入，依赖接口编程，而不是具体的实现。 数据对象适合在类中new 比如各种model do vo info 。 一家之言欢迎讨论指正。2020-01-08Jessica 👍（11） 💬（3）public class Demo { public long caculateDelayDays(Date dueTime，long currentTimestamp) { if (dueTime.getTime() &gt;= currentTimestamp) { return 0; } long delayTime = currentTimestamp - dueTime.getTime(); long delayDays = delayTime / 86400; return delayDays; }</p><pre><code>private long getCurrentTimestamp() {
    return System.currentTimeMillis(); 
}
</code></pre><p>}</p><p>mock掉getCurrentTimestamp（）, dueTime给定固定的时间，只检查逻辑是否符合预期2020-03-02Jesse 👍（11） 💬（0）思考题1，该方法产生一个唯一的ID,我认为不需要mock。 思考题2，我觉得如果对象有行为，并且行为与外部系统交互或者执行的结果具有不确定性，就需要依赖注入来完成测试。如果对象的行为是可预测的并且唯一的，可以直接new。2020-01-08小伟 👍（10） 💬（0）思考题：</p><ol><li>看IdGenerator怎么实现，如果要查数据库或分布式服务，那么会有影响，需要mock；如果是本地生产，不需要mock。</li><li>个人观点，不要在方法里new对象，依赖都要以注入的方式获取。2020-01-20</li></ol>`,86)])])}const g=n(l,[["render",i]]);export{m as __pageData,g as default};
