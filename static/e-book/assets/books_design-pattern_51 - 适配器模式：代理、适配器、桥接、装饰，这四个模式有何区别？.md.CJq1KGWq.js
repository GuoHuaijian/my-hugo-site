import{_ as n,o as a,c as p,ae as e}from"./chunks/framework.Iv6F95cJ.js";const u=JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"books/design-pattern/51 - 适配器模式：代理、适配器、桥接、装饰，这四个模式有何区别？.md","filePath":"books/design-pattern/51 - 适配器模式：代理、适配器、桥接、装饰，这四个模式有何区别？.md"}'),l={name:"books/design-pattern/51 - 适配器模式：代理、适配器、桥接、装饰，这四个模式有何区别？.md"};function i(t,s,c,r,o,d){return a(),p("div",null,[...s[0]||(s[0]=[e(`<p>前面几节课我们学习了代理模式、桥接模式、装饰器模式，今天，我们再来学习一个比较常用的结构型模式：适配器模式。这个模式相对来说还是比较简单、好理解的，应用场景也很具体，总体上来讲比较好掌握。</p><p>关于适配器模式，今天我们主要学习它的两种实现方式，类适配器和对象适配器，以及5种常见的应用场景。同时，我还会通过剖析slf4j日志框架，来给你展示这个模式在真实项目中的应用。除此之外，在文章的最后，我还对代理、桥接、装饰器、适配器，这4种代码结构非常相似的设计模式做简单的对比，对这几节内容做一个简单的总结。</p><p>话不多说，让我们正式开始今天的学习吧！</p><h2 id="适配器模式的原理与实现" tabindex="-1">适配器模式的原理与实现 <a class="header-anchor" href="#适配器模式的原理与实现" aria-label="Permalink to &quot;适配器模式的原理与实现&quot;">&amp;ZeroWidthSpace;</a></h2><p><strong>适配器模式</strong>的英文翻译是<strong>Adapter Design Pattern</strong>。顾名思义，这个模式就是用来做适配的，它将不兼容的接口转换为可兼容的接口，让原本由于接口不兼容而不能一起工作的类可以一起工作。对于这个模式，有一个经常被拿来解释它的例子，就是USB转接头充当适配器，把两种不兼容的接口，通过转接变得可以一起工作。</p><p>原理很简单，我们再来看下它的代码实现。适配器模式有两种实现方式：类适配器和对象适配器。其中，类适配器使用继承关系来实现，对象适配器使用组合关系来实现。具体的代码实现如下所示。其中，ITarget表示要转化成的接口定义。Adaptee是一组不兼容ITarget接口定义的接口，Adaptor将Adaptee转化成一组符合ITarget接口定义的接口。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>// 类适配器: 基于继承</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>针对这两种实现方式，在实际的开发中，到底该如何选择使用哪一种呢？判断的标准主要有两个，一个是Adaptee接口的个数，另一个是Adaptee和ITarget的契合程度。</p><ul><li>如果Adaptee接口并不多，那两种实现方式都可以。</li><li>如果Adaptee接口很多，而且Adaptee和ITarget接口定义大部分都相同，那我们推荐使用类适配器，因为Adaptor复用父类Adaptee的接口，比起对象适配器的实现方式，Adaptor的代码量要少一些。</li><li>如果Adaptee接口很多，而且Adaptee和ITarget接口定义大部分都不相同，那我们推荐使用对象适配器，因为组合结构相对于继承更加灵活。</li></ul><h2 id="适配器模式应用场景总结" tabindex="-1">适配器模式应用场景总结 <a class="header-anchor" href="#适配器模式应用场景总结" aria-label="Permalink to &quot;适配器模式应用场景总结&quot;">&amp;ZeroWidthSpace;</a></h2><p>原理和实现讲完了，都不复杂。我们再来看，到底什么时候会用到适配器模式呢？</p><p>一般来说，适配器模式可以看作一种“补偿模式”，用来补救设计上的缺陷。应用这种模式算是“无奈之举”。如果在设计初期，我们就能协调规避接口不兼容的问题，那这种模式就没有应用的机会了。</p><p>前面我们反复提到，适配器模式的应用场景是“接口不兼容”。那在实际的开发中，什么情况下才会出现接口不兼容呢？我建议你先自己思考一下这个问题，然后再来看我下面的总结 。</p><h3 id="_1-封装有缺陷的接口设计" tabindex="-1">1.封装有缺陷的接口设计 <a class="header-anchor" href="#_1-封装有缺陷的接口设计" aria-label="Permalink to &quot;1.封装有缺陷的接口设计&quot;">&amp;ZeroWidthSpace;</a></h3><p>假设我们依赖的外部系统在接口设计方面有缺陷（比如包含大量静态方法），引入之后会影响到我们自身代码的可测试性。为了隔离设计上的缺陷，我们希望对外部系统提供的接口进行二次封装，抽象出更好的接口设计，这个时候就可以使用适配器模式了。</p><p>具体我还是举个例子来解释一下，你直接看代码应该会更清晰。具体代码如下所示：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class CD { //这个类来自外部sdk，我们无权修改它的代码</span></span>
<span class="line"><span>  //...</span></span>
...
<span class="line"><span>    return maskedText;</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><h3 id="_3-替换依赖的外部系统" tabindex="-1">3.替换依赖的外部系统 <a class="header-anchor" href="#_3-替换依赖的外部系统" aria-label="Permalink to &quot;3.替换依赖的外部系统&quot;">&amp;ZeroWidthSpace;</a></h3><p>当我们把项目中依赖的一个外部系统替换为另一个外部系统的时候，利用适配器模式，可以减少对代码的改动。具体的代码示例如下所示：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>// 外部系统A</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p>所以，在开发业务系统或者开发框架、组件的时候，我们统一使用Slf4j提供的接口来编写打印日志的代码，具体使用哪种日志框架实现（log4j、logback……），是可以动态地指定的（使用Java的SPI技术，这里我不多解释，你自行研究吧），只需要将相应的SDK导入到项目中即可。</p><p>不过，你可能会说，如果一些老的项目没有使用Slf4j，而是直接使用比如JCL来打印日志，那如果想要替换成其他日志框架，比如log4j，该怎么办呢？实际上，Slf4j不仅仅提供了从其他日志框架到Slf4j的适配器，还提供了反向适配器，也就是从Slf4j到其他日志框架的适配。我们可以先将JCL切换为Slf4j，然后再将Slf4j切换为log4j。经过两次适配器的转换，我们就能成功将JCL切换为了log4j。</p><h2 id="代理、桥接、装饰器、适配器4种设计模式的区别" tabindex="-1">代理、桥接、装饰器、适配器4种设计模式的区别 <a class="header-anchor" href="#代理、桥接、装饰器、适配器4种设计模式的区别" aria-label="Permalink to &quot;代理、桥接、装饰器、适配器4种设计模式的区别&quot;">&amp;ZeroWidthSpace;</a></h2><p>代理、桥接、装饰器、适配器，这4种模式是比较常用的结构型设计模式。它们的代码结构非常相似。笼统来说，它们都可以称为Wrapper模式，也就是通过Wrapper类二次封装原始类。</p><p>尽管代码结构相似，但这4种设计模式的用意完全不同，也就是说要解决的问题、应用场景不同，这也是它们的主要区别。这里我就简单说一下它们之间的区别。</p><p>**代理模式：**代理模式在不改变原始类接口的条件下，为原始类定义一个代理类，主要目的是控制访问，而非加强功能，这是它跟装饰器模式最大的不同。</p><p>**桥接模式：**桥接模式的目的是将接口部分和实现部分分离，从而让它们可以较为容易、也相对独立地加以改变。</p><p>**装饰器模式：**装饰者模式在不改变原始类接口的情况下，对原始类功能进行增强，并且支持多个装饰器的嵌套使用。</p><p>**适配器模式：**适配器模式是一种事后的补救策略。适配器提供跟原始类不同的接口，而代理模式、装饰器模式提供的都是跟原始类相同的接口。</p><h2 id="重点回顾" tabindex="-1">重点回顾 <a class="header-anchor" href="#重点回顾" aria-label="Permalink to &quot;重点回顾&quot;">&amp;ZeroWidthSpace;</a></h2><p>好了，今天的内容到此就讲完了。让我们一块来总结回顾一下，你需要重点掌握的内容。</p><p>适配器模式是用来做适配，它将不兼容的接口转换为可兼容的接口，让原本由于接口不兼容而不能一起工作的类可以一起工作。适配器模式有两种实现方式：类适配器和对象适配器。其中，类适配器使用继承关系来实现，对象适配器使用组合关系来实现。</p><p>一般来说，适配器模式可以看作一种“补偿模式”，用来补救设计上的缺陷。应用这种模式算是“无奈之举”，如果在设计初期，我们就能协调规避接口不兼容的问题，那这种模式就没有应用的机会了。</p><p>那在实际的开发中，什么情况下才会出现接口不兼容呢？我总结下了下面这样5种场景：</p><ul><li>封装有缺陷的接口设计</li><li>统一多个类的接口设计</li><li>替换依赖的外部系统</li><li>兼容老版本接口</li><li>适配不同格式的数据</li></ul><h2 id="课堂讨论" tabindex="-1">课堂讨论 <a class="header-anchor" href="#课堂讨论" aria-label="Permalink to &quot;课堂讨论&quot;">&amp;ZeroWidthSpace;</a></h2><p>今天我们讲到，适配器有两种实现方式：类适配器、对象适配器。那我们之前讲到的代理模式、装饰器模式，是否也同样可以有两种实现方式（类代理模式、对象代理模式，以及类装饰器模式、对象装饰器模式）呢？</p><p>欢迎留言和我分享你的思考，如果有收获，也欢迎你把这篇文章分享给你的朋友。 精选留言（15） Laughing 👍（10） 💬（1）1. 代理模式中，委托类的实现基本上就是类代理的模式 2. 装饰器模式本身为了解决继承太深的问题，所以没有类装饰器的模式2020-11-20Obed 👍（2） 💬（1）王争老师 今天学习了这篇文章，你说slf4j会使用spi的技术动态指定具体使用哪一种框架。然后我查了一下资料，看了自己项目关于日志的源码。slf4j好像是指定了org.slf4j.impl这个包。然后在LoggerFactory.getLogger()的时候在去扫描实现了slf4j接口的日志的这个指定包去加载对应的类。这跟java的spi好像不大一样。还是说其实这种实现跟spi的思想都是一样的2020-07-14有爱有波哥 👍（0） 💬（1）CD 实现代码不是接口是具体的方法吗？2020-05-22javaadu 👍（120） 💬（6）这篇总结将前几节课串联起来了，非常赞👍</p><p>课堂讨论：</p><ol><li>代理模式支持，基于接口组合代理就是对象匹配，基于继承代理就是类匹配</li><li>装饰者模式不支持，这个模式本身是为了避免继承结构爆炸而设计的2020-02-28小晏子 👍（65） 💬（2）代理模式有两种实现方式：一般情况下，我们让代理类和原始类实现同样的接口。这种就是对象代理模式；但是，如果原始类并没有定义接口，并且原始类代码并不是我们开发维护的。在这种情况下，我们可以通过让代理类继承原始类的方法来实现代理模式，这种属于类代理模式。 装饰器模式没有这两种方式：装饰器模式主要解决继承关系过于复杂的问题，通过组合来替代继承，在设计的时候，装饰器类需要跟原始类继承相同的抽象类或者接口。所以装饰器只有对象装饰器这一种。 2020-02-28阿骨打 👍（35） 💬（8）说实话真的牛，看到51节，争哥的水平估计高于99.9%的码农了，能懂是一层境界，能说给别人听，使别人信服是一层境界，能串联起来说给别人听，又是一层境界。2020-09-18唐龙 👍（33） 💬（5）C++的STL里有大量的适配器，比如迭代器适配器，容器适配器，仿函数适配器。</li></ol><p>容器里的反向迭代器reverse_iterator就是对迭代器iterator的一层简单封装。</p><p>所谓的栈stack和单向队列queue也是对其他容器的封装，底层默认使用的是双向队列deque，两者也都可以选用双向链表list，stack也可以使用向量vector。可以通过模板参数选用具体的底层容器，比如stack&lt;int, vector&lt;int&gt;&gt; stk;。</p><p>而仿函数适配器functor adapter则是其中的重头戏，众所周知，仿函数functor是一种重载了函数调用运算符的类。仿函数适配器可以改变仿函数的参数个数，比如bind1st, bind2nd等。</p><p>一个使用仿函数适配器的例子： count_if(scores.begin(),scores.end(),bind2nd(less&lt;int&gt;(), 60)); 上述代码翻译成人话就是统计不到60分成绩的人数。</p><p>正常来讲，不论count_if的最后一个参数是函数指针还是仿函数对象，只能接受一个参数，我们没必要为“小于60”这么微不足道的事情单独写一个函数或是仿函数，所以选择了通过bind2nd这一个适配器改变函数的参数个数，并且把其中的第二个参数绑定为60。</p><p>STL使用适配器的目的是为了更灵活的组合一些基础操作，并不是设计缺陷。</p><p>所以对于老师所说的 ……适配器模式可以看作一种“补偿模式”，用来补救设计上的缺陷。应用这种模式算是“无奈之举”…… 我并不认同。2020-02-28勤劳的明酱 👍（18） 💬（7）那SpringAop是代理模式，主要功能却是增强被代理的类，这不是更符合装饰器模式。2020-02-28honnkyou 👍（16） 💬（1）1 中的代码ITarget应该是接口吧2020-03-17每天晒白牙 👍（11） 💬（1）代理模式有两种实现方式 1.代理类和原始类实现相同的接口，原始类只负责原始的业务功能，而代理类通过委托的方式调用原始类来执行业务逻辑，然后可以做一些附加功能。这也是一种基于接口而实现编程的设计思想。这就是基于组合也就是对象模式 2.如果原始类没有定义接口且不是我们开发维护的，这属于对外部类的扩展，可以使用继承的方式，只需要用代理类继承原始类，然后附加一些功能。这就是基于类模式</p><p>装饰者模式主要解决的问题就是继承关系过于复杂，通过组合来代替继承，主要作用是给原始类添加增强功能。所以装饰者模式只有对象模式2020-02-28f 👍（8） 💬（0）真的是好文章，讲出了别人讲不出的设计，看了很多遍理解了，但是想要融会贯通不是件容易的事，需要多实践，我需要反复看这边文章来思考这5种场景的实践，这次阅读的理解就是场景二可以指导框架设计，我们只要实现新的过滤器，add，就可以不修改源码来实现新的过滤，简直太棒了2020-02-29图灵机 👍（6） 💬（0） FutureTask 里的RunnableAdapter实现了适配器模式2020-10-01mqray 👍（4） 💬（0）最典型的就是Thread接口中只支持Runnable而不支持Callable 通过FutureTask适配2021-03-29王钰淇 👍（3） 💬（3）我觉得适配器模式还有一个很重要的作用是用来隔离影响，使适配器关联的两个类可以独立发展，影响仅局限在变化的类和适配器本身，另外一个类不需要关心。2020-04-20Monday 👍（3） 💬（1）代理、桥接、装饰器、适配器，这 4 种模式是比较常用的结构型设计模式。 记得桥接模式那节说桥接械不常用，熟悉即可。现在这边说成了常用了。2020-03-13</p>`,69)])])}const b=n(l,[["render",i]]);export{u as __pageData,b as default};
