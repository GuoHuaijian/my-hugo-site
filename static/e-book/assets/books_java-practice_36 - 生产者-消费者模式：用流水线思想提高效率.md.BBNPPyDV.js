...
<span class="line"><span>}</span></span></code></pre></div><h2 id="总结" tabindex="-1">总结 <a class="header-anchor" href="#总结" aria-label="Permalink to &quot;总结&quot;">&amp;ZeroWidthSpace;</a></h2><p>Java语言提供的线程池本身就是一种生产者-消费者模式的实现，但是线程池中的线程每次只能从任务队列中消费一个任务来执行，对于大部分并发场景这种策略都没有问题。但是有些场景还是需要自己来实现，例如需要批量执行以及分阶段提交的场景。</p><p>生产者-消费者模式在分布式计算中的应用也非常广泛。在分布式场景下，你可以借助分布式消息队列（MQ）来实现生产者-消费者模式。MQ一般都会支持两种消息模型，一种是点对点模型，一种是发布订阅模型。这两种模型的区别在于，点对点模型里一个消息只会被一个消费者消费，和Java的线程池非常类似（Java线程池的任务也只会被一个线程执行）；而发布订阅模型里一个消息会被多个消费者消费，本质上是一种消息的广播，在多线程编程领域，你可以结合观察者模式实现广播功能。</p><h2 id="课后思考" tabindex="-1">课后思考 <a class="header-anchor" href="#课后思考" aria-label="Permalink to &quot;课后思考&quot;">&amp;ZeroWidthSpace;</a></h2><p>在日志组件异步刷盘的示例代码中，写日志的线程以 <code>while(true){}</code> 的方式执行，你有哪些办法可以优雅地终止这个线程呢？</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>this.writer.execute(()-&gt;{</span></span>
<span class="line"><span>  try {</span></span>
<span class="line"><span>    //未刷盘日志数量</span></span>
<span class="line"><span>    int curIdx = 0;</span></span>
<span class="line"><span>    long preFT=System.currentTimeMillis();</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>  } catch(Exception e) {}</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>欢迎在留言区与我分享你的想法，也欢迎你在留言区记录你的思考过程。感谢阅读，如果你觉得这篇文章对你有帮助的话，也欢迎把它分享给更多的朋友。 精选留言（15） 锦 👍（140） 💬（1）在应用系统中，日志系统一般都是最后关闭的吧，因为它要为其他系统关闭提供写日志服务。所以日志系统关闭时需要把队列中所有日志都消费掉才能关闭。 可能需要在关闭日志系统时投入一个毒丸，表示没有新的日志写入。线程池在消费到毒丸时知道没有日志写入，将所有的日志刷盘，break循环体。 2019-05-21PK時頭髮不亂 👍（49） 💬（4）极客时间有好多课程, 我觉得王老师的干货是最实际最可用的, 必须要赞一个。2019-05-21êｗěｎ 👍（29） 💬（4） 之前遇到过一个生产问题，一个服务启动一段时间后就不停的超时，后面结合线程栈发现很多阻塞在打印日志的地方（我们用的就是log4j2），后面查到机子硬盘问题，io直接100%以上，日志刷盘满导致消费速度慢，队列撑满阻塞了写，这间接说明平衡好生产和消费速度以及适当的队列大小是很有必要。2019-05-22苏籍 👍（14） 💬（1）您好老师问个最近用到的线程池使用的问题 我的工程是springboot的，在unitTest里（@SpringBootTest） 里调用了一个service A（通过@Autowired的）中的方法，A中启用了一个线程池，执行的任务 是往数据库里插入数据。但是总抛出数据源已经被关闭的异常，我理解的是在单测主线程已经结束，所以关闭了数据源这些清理工作，而此时线程池的线程还 没结束，这个时候去调用数据源是null 的，不知道这么理解对不对，另外这个test主线程结束，为啥线程池的线程还没结束（通过打断点看到的）。这个怎么理解，求教2019-05-22Asanz 👍（12） 💬（1）看到很多示例代码都没有关闭线程池的动作，难道局部的线程池就不要关闭吗？2019-08-08生活发言权 👍（9） 💬（3）请问一下高并发场景，四个人拼一个团，怎么拼？ 1.db里记录拼团人数，如果小于4则直接update到拼团用户表，否则创建新的拼团id，新的记录。 2.高并发场景，怎么保证读写db的一致性？redis和db双写？ 3.期待老师高见2019-09-18berthav_ss 👍（8） 💬（1）宝令老师，如何优雅的停止线程池中某一组线程呢？例如我在线程a中启动了1-10线程，线程b中启动了2-30线程，如何优雅停止1-10线程呢2019-06-11来 👍（5） 💬（1）你好我想问下，生产者-消费者模式和worker thread模式有什么区别和联系？我看它们的核心都是任务队列，都是先把任务放到任务队列中，然后再从任务队列中获取任务并执行。2020-06-03Geek_b38255 👍（1） 💬（1）老师你好，想请教下，生产者-消费者模式 与 Worker Thread 模式的区别是什么呢？2021-05-01Monday 👍（1） 💬（1）第一段代码，非阻塞获取任务时，如果再单批次数量上限比如1000，是不是更好些2020-12-23荷兰小猪8813 👍（1） 💬（2）如果使用轻量级线程，就没有必要平衡生产者和消费者的速度差异了，因为轻量级线程本身就是廉价的，</p><p>为什么廉价就不需要平衡？2019-11-25Geek_0quh3e 👍（1） 💬（1）声明一个volatie变量用于表示线程结束，为true时，退出循环 2019-05-21密码123456 👍（1） 💬（1）设置一个volitle 。这里中断设置不了，没有引用。我觉得一个volite关键字够了。之前说happens before的时候说，volit写，优于volit读，应该立刻可见。还要问下老师，这么理解可以吗？中断是不是一定必须的？2019-05-21王藝明 👍（0） 💬（1）老师好！批量插入 SQL 的案例中，【首先是以阻塞方式获取任务队列中的一条任务，而后则是以非阻塞的...】下面的代码，感觉如果一直能取到内容，岂不是退不出循环体了2019-08-02尔东橙 👍（0） 💬（1）宝令老师，现在大公司代码规范都是缩进两空格么？2019-07-21</p>`,32)])])}const u=n(l,[["render",i]]);export{h as __pageData,u as default};
