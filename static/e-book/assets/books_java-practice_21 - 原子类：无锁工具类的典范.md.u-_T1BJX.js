...
<span class="line"><span>}while(!compareAndSet(oldV,newV);</span></span></code></pre></div><h2 id="原子类概览" tabindex="-1">原子类概览 <a class="header-anchor" href="#原子类概览" aria-label="Permalink to &quot;原子类概览&quot;">&amp;ZeroWidthSpace;</a></h2><p>Java SDK并发包里提供的原子类内容很丰富，我们可以将它们分为五个类别：<strong>原子化的基本数据类型、原子化的对象引用类型、原子化数组、原子化对象属性更新器</strong>和<strong>原子化的累加器</strong>。这五个类别提供的方法基本上是相似的，并且每个类别都有若干原子类，你可以通过下面的原子类组成概览图来获得一个全局的印象。下面我们详细解读这五个类别。</p><p><img src="https://static001.geekbang.org/resource/image/00/4a/007a32583fbf519469462fe61805eb4a.png?wh=1142%2A461" alt="" loading="lazy" referrerpolicy="no-referrer"></p><p>原子类组成概览图</p><h3 id="_1-原子化的基本数据类型" tabindex="-1">1. 原子化的基本数据类型 <a class="header-anchor" href="#_1-原子化的基本数据类型" aria-label="Permalink to &quot;1. 原子化的基本数据类型&quot;">&amp;ZeroWidthSpace;</a></h3><p>相关实现有AtomicBoolean、AtomicInteger和AtomicLong，提供的方法主要有以下这些，详情你可以参考SDK的源代码，都很简单，这里就不详细介绍了。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>getAndIncrement() //原子化i++</span></span>
...
<span class="line"><span>  int update)</span></span></code></pre></div><h3 id="_5-原子化的累加器" tabindex="-1">5. 原子化的累加器 <a class="header-anchor" href="#_5-原子化的累加器" aria-label="Permalink to &quot;5. 原子化的累加器&quot;">&amp;ZeroWidthSpace;</a></h3><p>DoubleAccumulator、DoubleAdder、LongAccumulator和LongAdder，这四个类仅仅用来执行累加操作，相比原子化的基本数据类型，速度更快，但是不支持compareAndSet()方法。如果你仅仅需要累加操作，使用原子化的累加器性能会更好。</p><h2 id="总结" tabindex="-1">总结 <a class="header-anchor" href="#总结" aria-label="Permalink to &quot;总结&quot;">&amp;ZeroWidthSpace;</a></h2><p>无锁方案相对于互斥锁方案，优点非常多，首先性能好，其次是基本不会出现死锁问题（但可能出现饥饿和活锁问题，因为自旋会反复重试）。Java提供的原子类大部分都实现了compareAndSet()方法，基于compareAndSet()方法，你可以构建自己的无锁数据结构，但是<strong>建议你不要这样做，这个工作最好还是让大师们去完成</strong>，原因是无锁算法没你想象的那么简单。</p><p>Java提供的原子类能够解决一些简单的原子性问题，但你可能会发现，上面我们所有原子类的方法都是针对一个共享变量的，如果你需要解决多个变量的原子性问题，建议还是使用互斥锁方案。原子类虽好，但使用要慎之又慎。</p><h2 id="课后思考" tabindex="-1">课后思考 <a class="header-anchor" href="#课后思考" aria-label="Permalink to &quot;课后思考&quot;">&amp;ZeroWidthSpace;</a></h2><p>下面的示例代码是合理库存的原子化实现，仅实现了设置库存上限setUpper()方法，你觉得setUpper()方法的实现是否正确呢？</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class SafeWM {</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>欢迎在留言区与我分享你的想法，也欢迎你在留言区记录你的思考过程。感谢阅读，如果你觉得这篇文章对你有帮助的话，也欢迎把它分享给更多的朋友。 精选留言（15） 张天屹 👍（113） 💬（3）如果线程1 运行到WMRange or = rf.get();停止，切换到线程2 更新了值，切换回到线程1，进入循环将永远比较失败死循环，解决方案是将读取的那一句放入循环里，CAS每次自旋必须要重新检查新的值才有意义2019-04-16Geek_ebda96 👍（45） 💬（19）老师你举的这个例子，自己实现CAS是不是有点不对 class SimulatedCAS{ volatile int count; // 实现 count+=1 addOne(){ do { newValue = count+1; //① }while(count != cas(count,newValue) //② } // 模拟实现 CAS，仅用来帮助理解 synchronized int cas( int expect, int newValue){ // 读目前 count 的值 int curValue = count; // 比较目前 count 值是否 == 期望值 if(curValue == expect){ // 如果是，则更新 count 的值 count= newValue; } // 返回写入前的值 return curValue; } }</p><p>2 这里是不是应该用oldValue来比较，在do里面的时候先把count的值用oldValue保存下来，传入的参数expected为oldValue,newValue为oldValue+1</p><p>do{ oldValue = count; newValue = oldValue + 1; }while(oldValue != cas(oldValue, newValue))</p><p>望指正2019-04-18长脖子树 👍（26） 💬（4）原子类在 java 层面虽然看起来是无锁的, 但是深入到操作系统和cpu层面仍然有锁 比如像在多处理器计算机里常常会有一种 TSL 指令, (测试并加锁 test and set lock), 它是一种硬件支持的互斥方案, 执行这个指令的cpu将锁住内存总线, 以禁止其他CPU 在本指令结束之前访问内存 这个指令类似于 intel 处理器上的 lock cmpxchg , 但在近几代的实现上, 对内存总线的锁定做了类似于分段锁的优化, 仅仅锁定部分的缓存行 参考:</p><ol><li>现代操作系统</li><li>https://en.wikipedia.org/wiki/Compare-and-swap</li><li>https://stackoverflow.com/questions/11065675/lock-prefix-of-intel-instruction-what-is-the-point</li><li>https://cloud.tencent.com/developer/article/11898842020-04-25木卫六 👍（13） 💬（2）首先，or=rf.get()需要放到do{}，每次需要重新获取，以防其他线程更新过导致死循环；</li></ol><p>然后，nr是new的，我觉得应该不会发生ABA的问题（reference的compareAndSet比较的是内存地址）。另外ABA问题应该容易发生在值类型上吧，引用类型的应该几乎不会发生？对于引用类型，几乎不会发生经过至少两次new对象，最后对象放在了同一块or之前使用的内存区块上吧？ 2019-04-16andy 👍（12） 💬（4）public class SafeWM { class WMRange{ final int upper; final int lower; WMRange(int upper,int lower){ // 省略构造函数实现 } } final AtomicReference&lt;WMRange&gt; rf = new AtomicReference&lt;&gt;( new WMRange(0,0) ); // 设置库存上限 void setUpper(int v){ WMRange nr; WMRange or; do{ or = rf.get(); // 检查参数合法性 if(v &lt; or.lower){ throw new IllegalArgumentException(); } nr = new WMRange(v, or.lower); }while(!rf.compareAndSet(or, nr)); } }</p><p>这样子对吗？2019-04-16郑晨Cc 👍（12） 💬（3）or是原始的 nr是new出来的 指向不同的内存地址 compareandset的结果永远返回false 结果是死循环？是不是应该用atomicfieldreference？2019-04-16随风而逝 👍（7） 💬（3）老师，这些原子操作类在分布式程序中还有效吗？2019-04-30刘志兵 👍（7） 💬（1）老师，compareAndSwapLong方法是一个native方法，比较共享变量和expect值是否相等，相等才设置新的值x, 不明白这里的对比是怎么保证原子性的，对比也是要再读一次共享变量，然后对比吧，如果先读出来之后对比的时候被其他线程修改了，那还是会有问题2019-04-17刘育飞 👍（6） 💬（1） 不明白 synchronized int cas() 这不是已经用了 同步synchronized 关键字 吗怎么会 无锁 无堵塞呢2019-10-17忍者无敌1995 👍（6） 💬（1）例子中的模拟CAS，cas函数是加了锁的，保证整个操作的原子性；我的理解是这个只是一个模拟，实际中肯定不会加上锁的2019-05-02Vincent 👍（5） 💬（1）第一个例子也不是线程安全的吧？i++这个操作不是线程安全的，会导致判断错误吧？2019-06-25考休 👍（4） 💬（1）cas的实现原理感觉跟乐观锁有相似的地方，不知道是不是可以这么理解2019-05-16密码123456 👍（4） 💬（2）我觉得可能会出现死循环。WMRange or = rf.get(); 应该放在do里面。每次比较交换失败后，重新获取一次。2019-04-16随心而至 👍（3） 💬（1）感觉理解好了volatile和CAS，这些原子类就都好理解了2019-09-05linqw 👍（3） 💬（1）课后习题：如果在do{}while()第一次没设置成功，即对象已经被其他线程修改，or已经是过期的对象，导致死循环，可以写成如： public class SafeWM { class WMRange{ final int upper; final int lower; WMRange(int upper,int lower){ if(upper &lt; lower){ throw new IllegalArgumentException(); } // 省略构造函数实现 } } final AtomicReference&lt;WMRange&gt; rf = new AtomicReference&lt;&gt;( new WMRange(0,0) ); // 设置库存上限 void setUpper(int v){ WMRange nr; WMRange or; do{ or = = rf.get(); nr = new WMRange(v, or.lower); }while(!rf.compareAndSet(or, nr)); } }2019-04-21</p>`,62)])])}const h=a(l,[["render",t]]);export{g as __pageData,h as default};
